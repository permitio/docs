...
In the page that handles the link, confirm the user's identity, and then approve the invite from your backend. The invite code is the `user_invite_id`. This example uses the [Python SDK](/sdk/python/quickstart-python). Replace the token with your API key, the email and key with the invited user's email and user key, and `user_invite_id` with the invite code:
