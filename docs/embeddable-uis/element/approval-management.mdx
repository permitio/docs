...
After the request will approve, the requesting user will be automatically assigned to the approved role (on the relevant resource instance for his request).
...
:::note
Note that it is crucial to select the tenant relevant for this element for it to function properly.
:::
