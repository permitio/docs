...
## Approval Flows Element
This element consists of two steps, the first step is the user management and the second step is in the access request element.
...
For example, a normal user cannot write blog posts, but an editor can write and edit posts. We show the access request element to a normal user when he reaches the post editing screen. He asks the admin for access to the blocked resource there.
In the user management element, the administrator can grant or deny access to the user based on the request.
...
<div className="list">
    <ul>
        ...
        <li>
			<span>
				Give the element a title, message and text in button, that makes sense to your end user.
			</span>
        </li>
    </ul>
</div>
...
### Managing access requests with user management element
In element of user management there is a section **Approval Component in user management** element you can connect a access request component.
