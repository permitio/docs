...
Use the Access Request API to create requests for specific roles or resource instances.

It also assigns relevant moderators to approve or deny user requests based on your decision
...
curl 'https://api.permit.io/v2/facts/{project_id}/{env_id}/access_requests' \
  -H 'cookie': from LoginMethod.cookie
