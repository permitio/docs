...
# Approval Request Element

<span className="subtitle-custom">
The Approval Request Element is a Frontend component that allows users to request approval for a specific action in your application.
When a request is made, it is sent to the workspace owner, who can either approve or deny it.
<br/>
Once the request is approved, the user will get an approval for the requested action, and in addition to that, a Webhook will be triggered to notify your application about the approval or denial of the request.
...
<span style={{margin:'0 30px'}}>
    <img src="/img/approval-request/approvalRequest.png" alt="ar-element" style={{width:'70%'}}/>
</span>

## Overview

The Approval Request Element allows users to submit approval requests through a dedicated UI embedded in your application.
This allows your, as the administrator of your application, to restrict your end-users to act on specific objects and resources, and require an approval process before these actions are getting executed.
After a user submits a request, it is sent for approval by an admin, who can either approve or deny it.

This element is especially useful when there are critical actions in your system that need an approval by a higher authority before they are executed.

Using the "Approval Request" Element is as simple as embedding it into your Frontend application. Once embedded, users can request approval to restricted resources by clicking the "Request Approval" button.
Later, an admin can view a list of all pending access requests in the "Approval Page" Element, where they can approve or deny them.

**Example**

Say we have a bank where users can transfer funds to other users. We want to ensure that all transfers above a certain amount are approved by a manager before they are executed.
In this case, we can use the "Approval Request" Element to allow users to request approval for large transfers.

Upon receiving this approval request, the manager reviews it to determine whether to approve this transfer request or not.

TODO DIAGRAM

[//]: # (![access-request]&#40;/TODO/ADD/IMAGE&#41;)


## Creating and Connecting the Element

### 1. In the [Elements Screen](https://app.permit.io/elements), under  TODO change____________, click on "Create Element".

![create-a-r-element](/img/approval-request/elementScreen.png)

### 2. Select the Resource to which you want to connect this Approval Request Element

![create-a-r-element](/img/approval-request/approval-r-beforeSelectResource.png)
...
![after-selected-um](/img/approval-request/approval-r-afterSelectResource.png)
