...
The Access Request feature allows users to request access to restricted resources. The request is sent to the admin, who can approve or deny it.
</span>
<br/>
<br/>
...
Access Request gives us news capability in our app. It's easy to allow or deny access requests that our users request.

**For example**

A normal user can't write a blog article, but an editor can. When a normal user wants to write an article, he requests access to the screen. When the system administrator gets his access request (in the user management element), he decides whether to grant him access or not.

Access Request requires a user management element. The reason it's a pre-request is that, when we create an access request, the workspace administrator can see it in his user management element - where he manages the list of users and those waiting for access approval.
