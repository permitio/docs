...
Permit Element is now compatible with private browsers, such as Chrome Incognito Mode and Safari Private Browsing.

Backend Configuration
To enable support for private browsing, add the following code to your backend:
...
```js
    ...
    app.post("/login_private_browser", async (req, res) => {
        const ticket = await permit.elements.loginAs({userId: user_key, tenantId: TENANT});
        res.status(200).json({url: ticket.login_cookie});
    });
    app.listen(port, () => {
        console.log(`Example app listening at http://localhost:${port}`);
    });
