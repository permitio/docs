...
| `INVALID_PERMISSION_LEVEL` | The user's role has no access to the part of the element being opened. A role left in **Hidden Roles** is the usual cause. | Drag the role to a permission level. See [Permission levels](/embeddable-uis/permission-levels). |
| `FORBIDDEN_ACCESS` | The API key that the backend login route uses is not an environment API key. A project-level or organization-level key is refused here. | Use the environment API key of the environment the element belongs to. See [Get your API key](/overview/get-api-key). |
