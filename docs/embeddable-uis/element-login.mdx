...
Choose the tab that matches your authentication method, then your backend language. In each example, replace the commented `get_user_from_jwt` placeholder, or the `<USER_KEY>` placeholder in the .NET and Java examples, with the code that reads the user key from your own session, and set the tenant variable or `<TENANT_KEY>` to the tenant the element shows.
