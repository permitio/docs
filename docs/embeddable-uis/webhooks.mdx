...
Now - when webhook notification only is on (email notification is off) <br />
Soon - when webhook notification is on, will behave like [Email Notification](/embeddable-uis/email-configuration-and-templates#supporting-users-invitation-and-approval-user-management-element) (add the user to the pending user list)
```python
{
    email: str,
    tenant_key: str,
...
Now - when both webhook and email notification is on: <br />
Coming soon - when webhook notification is on (with or without email config), <br /> will send an invite instead of create
```python
{
    email: str
    role: str
