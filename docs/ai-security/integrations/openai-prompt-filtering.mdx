...
The `Permit` client reads the PDP URL from `PDP_URL`. The Node.js SDK sets `throwOnError` to `true` by default, so a PDP that is unreachable or rejects the API key makes `permit.check()` throw instead of returning `false`. The `catch` block logs that error and rethrows it, which keeps a connection failure from looking like a denial. The version in the example repository is [`src/permit.js`](https://github.com/permitio/permit-prompt-filtering/blob/main/src/permit.js).
