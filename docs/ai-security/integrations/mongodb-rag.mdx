...
The response is a JSON object with `answer` and `sources`. The budget document is `docs/finance/budget_2024.md`, and `user_marketing_1` is a member of the marketing department, so the answer doesn't use finance documents.

Run the same query as `user_finance_1` to see an answer that uses the finance budget document:
