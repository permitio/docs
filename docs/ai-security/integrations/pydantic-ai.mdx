...
6. Run the agent as the restricted user. Change `user_id` in the `PermitDeps(...)` call in `example/main.py` from `user@example.com` to `restricted@example.com`, then run `uv run python example/main.py` again. The first response now carries the denial string from `validate_financial_query` instead of advice, and `disclaimer_added` stays `False`.
