...
| An [Anthropic](https://www.anthropic.com/) API key | PydanticAI reads the key from `ANTHROPIC_API_KEY`. Set the model string in `example/main.py` to a model that is Active on the [Claude model deprecations page](https://platform.claude.com/docs/en/about-claude/model-deprecations), such as `anthropic:claude-sonnet-4-6`. See step 5 of [Run the agent and verify the perimeters](#running-the-agent--testing-behavior). To use [OpenAI](https://openai.com/) instead, set the model string to an OpenAI model. |
...
8. Open [audit logs](/how-to/use-audit-logs/types-and-filtering) in the Permit dashboard. Each run adds one entry per check, so you can see which tool ran which check and whether the PDP allowed it. Try these prompts and compare the entries between the two users:
