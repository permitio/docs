...
| Retrieval-augmented generation (RAG) data protection | The LLM only receives documents the user can `view` | `Permit.filter_objects` |
...
Create `scripts/make_test_jwt.py`. The script creates an RSA key pair on the first run, writes the public key to `jwks/public/.well-known/jwks.json`, and prints a signed token for the `eligible` (`user-123`) or `ineligible` (`user-456`) test user:
...
Create `src/perimeters/rag_security.py`. Each document stores its key (`id`), resource type, and ABAC attributes in its metadata. `get_relevant_documents` passes the key and attributes to `filter_objects` and keeps the documents whose keys come back:
