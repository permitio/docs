...
description: The Consent Service is the user-facing component where humans authenticate, select MCP servers, set trust levels, and authorize agents — the complete user journey explained.
sidebar_position: 6
---

# Consent Service

The Consent Service is the user-facing component of Agent Security where humans authenticate and grant permissions to their AI agents. It serves as the OAuth 2.1 Authorization Server hosted at each gateway subdomain (e.g., `acme-pink-panda-6942.agent.security`).
...
After signing in, the consent screen shows a list of MCP servers the admin has [granted the user access to](/ai-security/mcp-permissions/guide#4-grant-user-access-to-mcp-servers). The user selects the server they want to connect to from this pre-approved list — they cannot enter an arbitrary URL.

![Consent flow — select MCP server](/img/agent-security/consent-flow/select-server.png)

:::note No servers?
If the user has not been granted access to any MCP server by the admin, they will see an empty state and won't be able to proceed. The admin must [grant access](/ai-security/mcp-permissions/guide#4-grant-user-access-to-mcp-servers) first.
...
The consent screen displays all discovered tools from the selected MCP server, organized by trust level. The user chooses the trust level they want to grant their agent, up to the maximum level the admin configured:
...
1. **Permissions sync to Permit** — the Consent Service creates the necessary relationships in Permit.io:
   ...
   - The user's profile-to-server relation is confirmed
...
When the user selects an MCP server, the Consent Service connects to the upstream server and calls `list_tools` to discover all available tools. Each tool is automatically classified into a trust level based on naming patterns — the same classification the admin saw during [MCP server import](/ai-security/mcp-permissions/platform#tool-auto-discovery-and-trust-level-classification). The consent screen then shows these tools grouped by trust level so the user understands exactly what they're granting.
...
- **Profile-to-server relation**: Confirms the human's access to the MCP server (set by the admin during [access granting](/ai-security/mcp-permissions/guide#4-grant-user-access-to-mcp-servers))

The effective trust level for any tool call is `min(agent_role, profile_relation)` — the human's admin-granted max trust acts as a ceiling on what any agent can actually exercise. See [Architecture: Trust Ceiling](/ai-security/mcp-permissions/architecture#authorization-trust-ceiling-min-logic) for the full derivation.
...
| ----------- | -------- | ------------ |
| **Inactivity timeout** | 90 days of inactivity | If no tool calls are made for 90 days, the session is automatically removed from Redis. Each tool call resets this timer, so actively used sessions do not expire. The user must re-consent to create a new session. |
| **Manual revocation** | Immediate | An admin can revoke access from the [Agents](/ai-security/mcp-permissions/managing-humans-and-agents#revoking-an-individual-agent) or [Humans](/ai-security/mcp-permissions/managing-humans-and-agents#revoking-a-humans-access) page, which immediately invalidates the session. |

:::info Session vs. consent permissions
The inactivity timeout applies to the **MCP transport session** (stored in Redis). Consent permissions (role assignments in Permit.io) persist independently — they are not affected by session expiry and remain active until explicitly revoked by an admin.
:::
...
The `upstream_mcp` parameter only pre-selects the server — it does not bypass access control. The user must still have been [granted access](/ai-security/mcp-permissions/guide#4-grant-user-access-to-mcp-servers) to the specified server by an admin. If they haven't, the pre-selection is ignored and they see only the servers they have access to.
:::

Admins can set up pre-configured URLs from the [Platform](/ai-security/mcp-permissions/platform) — the server detail page shows the upstream URL that can be used as the `upstream_mcp` value. See the [Host Setup Guide](/ai-security/mcp-permissions/host-setup#pre-selected-mcp-server-urls) for more on distributing these URLs to teams.
