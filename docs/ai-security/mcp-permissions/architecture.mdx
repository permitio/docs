...
Agent Security is available as a **hosted gateway** at `*.agent.security` or as a **self-hosted deployment** in your own Kubernetes cluster. Each host (tenant) gets a unique subdomain with isolated policies, users, and sessions.

| Deployment | When to use | Status |
| --- | --- | --- |
| **Hosted Gateway** | Fastest rollout; SaaS workloads | Available |
| **Self-Hosted (On-Prem)** | Data residency, compliance, air-gapped environments | Available ([details](/ai-security/mcp-permissions/on-prem-deployment)) |

Self-hosted deployments include a Helm chart, an on-prem PDP for local authorization decisions, and support for external secrets integration. See the [On-Prem Deployment](/ai-security/mcp-permissions/on-prem-deployment) page for details.
