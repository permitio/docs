...
Key components:
- **Gateway**: Managed proxy/gateway at `*.agent.security`
...
2. **Medium trust** — write operations (`create_*`, `write_*`, `update_*`, `edit_*`, `add_*`, `send_*`)
3. **High trust** — destructive operations (`delete_*`, `remove_*`, `destroy_*`, `erase_*`, `truncate_*`, `kill_*`)
...
- A `user_profile` resource that links each user to the agents acting on their behalf, enabling relationship-based permission derivation
...
5. The gateway issues a JWT access token to the MCP client
...
- **Human users**: `human|{subject}` — where `subject` is the authenticated user's identity
- **Agents**: `agent|{client_id}` — where `client_id` identifies the agent acting on behalf of the user
...
sequenceDiagram
    ...
    Note over A,CS: Admin setup (one-time)
    A->>G: Create host + import MCP servers
    A->>G: Grant user access to MCP servers

    Note over U,CS: User connection
...
:::note
The table below shows example configurations for illustration purposes. In practice, agent roles are automatically derived from trust levels selected during the user consent flow.
:::
