...
5. **User** picks an MCP server from their allowed list, chooses a trust level, and clicks **Authorize Agent**

   :::info
   By the time the user clicks **Authorize Agent**, the upstream OAuth with the external provider is already complete. Clicking **Authorize Agent** issues the authorization code to the MCP client, finalizing the consent for the selected trust level.
   :::
