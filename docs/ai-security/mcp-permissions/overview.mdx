...
2. **Admin** imports upstream MCP servers and defines the trust policy for each tool
3. **Admin** grants specific users permission to connect to specific MCP servers
4. **User** connects their MCP client (e.g., Cursor) to the gateway and signs in
5. **User** picks an MCP server from their allowed list, chooses a trust level, and consents
...
![Example UI Flow](/img/agent-security/authorize_agent_screen.png)

:::info
During the consent flow, the user selects an MCP server from their allowed list, chooses a trust level (up to the maximum the admin configured), and accepts. The gateway then issues a JWT access token to the MCP client and redirects back to the agent's configured callback URL.
:::
