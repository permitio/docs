...
description: Explore Agent Security's advanced capabilities — from behavioral fingerprinting and shadow agent detection to human-in-the-loop approvals and time-limited consent.
sidebar_position: 4
---
...
## Agent Fingerprints via Interrogation *(coming soon)*

Agent fingerprinting will go beyond OAuth tokens and API keys to verify _what_ an agent actually is — not just what it claims to be. When an agent connects, the gateway will run a lightweight behavioral interrogation to build a unique fingerprint that confirms the agent's identity.

**Why it matters:** Credentials can be stolen or shared, but behavioral characteristics are much harder to forge. Fingerprinting will let you detect impersonation, credential reuse across different agents, and unexpected behavioral drift — all before a single tool call is authorized.
...
## Inject Security Snitch Skills *(coming soon)*

Security Snitch Skills will be lightweight monitoring capabilities injected directly into an agent's active session. Rather than only observing traffic at the gateway boundary, these injected skills will give the security layer in-session visibility — enabling real-time anomaly detection, context tracking, and policy enforcement from within the agent's own tool environment.

**Why it matters:** Gateway-level inspection can only see individual requests in isolation. Injected monitoring skills will observe the full session context — tracking how an agent chains tool calls, what data it accumulates, and whether its behavior pattern matches its declared purpose.
...
## Shadow Agent Detection *(coming soon)*

Shadow agents are unregistered or unauthorized AI agents that connect to your MCP servers outside of approved channels. Shadow Agent Detection will use behavioral analysis and traffic pattern recognition to identify these rogue connections — even when they don't authenticate through the gateway.
...
## Human in the Loop (HITL) *(coming soon)*
...
## Workflow Intent Descriptions *(coming soon)*
...
## Time-Limited Consent *(partially available)*
...
MCP sessions currently have a **90-day inactivity timeout** — if no tool calls are made for 90 days, the session is automatically removed from Redis. Each tool call resets this timer, so actively used sessions do not expire. Note that this controls the **MCP transport session** lifetime, not the consent decision itself — permissions stored in Permit.io persist until explicitly revoked. Future releases will add a soft inactivity timeout (e.g., 30-day re-confirmation prompt), admin-configurable durations, per-tool expiration policies, and automatic consent downgrade workflows.
:::
