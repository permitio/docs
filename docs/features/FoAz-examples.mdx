...
## Check out FoAz Proxy Config API
...
3. If you have a url that is matched by multiple mapping rules, for example `https://api.stripe.com/v1/customers?limit={limit}` and `https://api.stripe.com/v1/customers`
    the order is randomly unless you provide the priority to the mapping rule ( the default priority used is 0 ).
4. The auth mechanism can be either Bearer, Basic, or Headers.
...
5. If you don't provide the action to the mapping rule, the http_method will be used as default.
