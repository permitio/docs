...
We could actually achieve the same result with less api calls.
...
Let's grant anyone who has the `account#admin` role the `file#editor` role on any file in that account:
