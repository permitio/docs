...
Permit MCP Gateway supports three deployment models. For a full comparison, see the [Enterprise Deployment](/permit-mcp-gateway/enterprise-deployment#deployment-models) guide.

- **Hosted (SaaS)** — managed by Permit at `*.agent.security`. Fast adoption, no infrastructure to manage. Available on all plans.
- **Customer-Controlled** — gateway and PDP run in your environment (VPC, on-prem). MCP traffic stays within your network. Permit.io remains the control plane.
- **Fully On-Premises** — the entire stack, including the control plane, runs in your environment. Supports air-gapped environments with no internet connectivity.

:::note
Customer-controlled and fully on-premises deployments are available on Enterprise plans. [Schedule a demo](https://calendly.com/permit-io/demo) to discuss your requirements. The hosted deployment is available for all plans and is the recommended starting point.
:::
...
See the full [Enterprise Deployment](/permit-mcp-gateway/enterprise-deployment) guide for deployment models, enterprise security controls, and air-gapped support.

:::note
Customer-controlled and fully on-premises deployments are available on Enterprise plans. [Schedule a demo](https://calendly.com/permit-io/demo) to discuss your requirements, or start with the [hosted gateway](/permit-mcp-gateway/quickstart) to evaluate — policies and users can be migrated between deployment models.
:::
