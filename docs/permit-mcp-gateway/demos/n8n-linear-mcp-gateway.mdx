...
A walkthrough demonstrating how to set up Permit MCP Gateway so that you can trust automated workflows built with n8n.
...
2. [Create a Permit MCP Gateway account](/permit-mcp-gateway/guide)
...
2. Insert the URL from the Linear MCP you created previously in the Permit MCP Gateway. For **Authentication**, select **MCP OAuth2** and then **Create New Credential**.
...
If at any time you want to change what n8n can do with the Linear MCP, go to the Permit MCP Gateway dashboard and toggle the trust level.

![Permit MCP Gateway dashboard showing trust level controls](/img/mcpermit/n8n-linear-demo/trust-level-dashboard.png)
