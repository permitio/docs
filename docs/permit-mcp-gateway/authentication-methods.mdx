...
description: Configure how users sign in to Permit MCP Gateway — email/password, OTP, passkeys, social OAuth, SAML 2.0, and OIDC SSO with IdP-specific setup instructions.
sidebar_position: 4.5
---

# Authentication Methods Reference

Each Permit MCP Gateway host has its own authentication configuration, controlling how users sign in during the [consent flow](/permit-mcp-gateway/guide#7-user-consent-flow). Authentication is per-tenant — different hosts can use different methods, credentials, and domain restrictions.
...
5. Under **Authorized redirect URIs**, add:
   ...
   ```
   Replace `{subdomain}` with your host's full subdomain (e.g., `acme-brave-coral-37`)
6. Copy the **Client ID** and **Client Secret**
7. In the Permit MCP Gateway dashboard, go to your host's **Settings** and enable **Google OAuth**
...
3. Fill in:
   - **Application name:** e.g., "Permit MCP Gateway — acme"
...
6. In the Permit MCP Gateway dashboard, go to your host's **Settings** and enable **GitHub OAuth**
...
3. Fill in:
   - **Name:** e.g., "Permit MCP Gateway — acme"
...
7. In the Permit MCP Gateway dashboard, go to your host's **Settings** and enable **Microsoft OAuth**
8. Paste the Client ID and Client Secret
...
Enterprise single sign-on via SAML. Permit MCP Gateway acts as the **Service Provider (SP)** and your identity provider (Okta, Azure AD/Entra ID, Google Workspace, etc.) acts as the **Identity Provider (IdP)**.
...
3. Fill in:
   - **App name:** e.g., "Permit MCP Gateway"
...
8. In the Permit MCP Gateway dashboard, go to your host's **Settings** and enable **SAML 2.0 SSO**
...
3. Name it (e.g., "Permit MCP Gateway") and click **Create**
...
8. In **Set up Permit MCP Gateway**, copy:
   - **Login URL** (entry point)
   - **Azure AD Identifier** (entity ID)
9. In the Permit MCP Gateway dashboard, go to your host's **Settings** and enable **SAML 2.0 SSO**
...
2. Name it (e.g., "Permit MCP Gateway") and click **Continue**
...
Permit MCP Gateway auto-discovers your IdP's endpoints using the standard OpenID Connect Discovery URL:

```
{issuerUrl}/.well-known/openid-configuration
```

You only need to provide the issuer URL — the gateway handles the rest.
...
5. In the Permit MCP Gateway dashboard, go to your host's **Settings** and enable **OIDC SSO**
