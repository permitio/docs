...
sequenceDiagram
    ...
    Note over GW: Host: acme-pink-panda-6942.agent.security

    GW->>R: GET host_lookup:acme-pink-panda-6942
...
By default, users can only connect to MCP servers that the admin has pre-configured (static servers). **Dynamic MCPs** is an opt-in feature that allows users to enter any upstream MCP server URL during the consent flow — useful for teams that need flexibility to connect to ad-hoc or personal MCP servers.

### How it works

1. An admin enables the **Allow Dynamic MCPs** toggle on the host settings page in the Platform
2. During the consent flow, users see an option to enter a custom MCP server URL in addition to the pre-configured server list
3. The user manages their own trust level for the dynamic server (there is no admin-set ceiling for dynamic servers)
4. The Consent Service creates per-user Permit resources for dynamic MCP servers, keyed as `{serverKey}-{userId}` with an `mcp_server_type: "dynamic"` attribute to distinguish them from static servers

### When to use

- **Development teams** that frequently spin up new MCP servers and need quick access without waiting for admin provisioning
- **Power users** who need to connect to niche or personal MCP servers
- **Evaluation scenarios** where users are testing multiple MCP servers before requesting admin provisioning

### Security considerations

- Dynamic MCPs bypass the admin allow-list — users can connect to any URL
- The admin can disable dynamic MCPs at any time by toggling the setting off
- All dynamic MCP tool calls are still authorized via Permit and logged in the audit trail
- The host-level `allow_dynamic_mcps` policy is enforced via a `connect_dynamic_mcp` action on the `user_profile` resource in Permit
