...
The gateway is protected by an AWS Web Application Firewall (WAF) attached to the load balancer. The WAF provides two layers of protection: managed rules that detect common attack patterns, and per-IP rate limits that prevent abuse.
...
| --- | --- | --- | --- | --- |
| `/api/auth/sign-in` | POST | 100 | 5 min | Brute-force and credential-stuffing protection |
| `/api/auth/sign-up` | POST | 100 | 10 min | Mass account creation prevention |
| `/oauth/register` | POST | 100 | 1 min | Dynamic Client Registration abuse prevention |
| `/mcp` | POST | 1000 | 1 min | MCP endpoint protection (~30 concurrent agents behind a corporate NAT) |
| All write operations | POST/PUT/DELETE | 1200 | 1 min | Global write ceiling |
| All traffic | All methods | 2000 | 1 min | Final catch-all (includes GET for SSE, discovery, UI) |
...
AWS Managed Rule groups provide defense-in-depth against common web attacks:
...
Request body inspection rules run in **monitoring mode** (count, not block) because MCP JSON-RPC payloads legitimately contain code snippets, SQL queries, and HTML that trigger false positives. Header, URI, and query string inspection rules are active and blocking.
:::
