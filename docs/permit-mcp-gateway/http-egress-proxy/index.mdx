...
| **Agent identity & drift protection** | Each agent session registers its task intent; a sharp change in declared intent blocks the session until it is re-approved — by the person who authorized the agent, in the browser (opt-in). See [Security](./security.mdx#agent-identity-and-drift-protection). |
...
- **Control plane** — you enable the proxy for a host, define workflows, store credentials, and mint agent tokens. You do this in the [dashboard](https://app.agent.security) or with the [`asg` CLI](./cli.mdx) (workflows are authored in the dashboard). It writes policy; it never carries live agent traffic.
- **Data plane** — your agent's runtime is configured with standard `HTTP_PROXY` / `HTTPS_PROXY` environment variables pointing at the gateway, plus a short-lived access token. From then on, the agent's normal HTTP libraries route through the proxy automatically. See [Connecting Agents](./connecting-agents.mdx).
...
For the detail behind each gate, see [Authorization & Trust](./authorization.mdx) and [Security](./security.mdx). Hosts with the opt-in [agent identity capability](./security.mdx#agent-identity-and-drift-protection) enabled additionally require a registered agent session before governed traffic flows.

---

## Start here

1. [**Quick Start**](./quickstart.mdx) — enable the proxy, define a workflow, store a credential, and route an agent through it.
2. [**The `asg` CLI**](./cli.mdx) — install and use the command-line tool that drives the control plane.
3. [**Workflows & Rules**](./egress-rules.mdx) — the workflow model: domains, paths, methods, and trust levels.
4. [**Credentials & Connections**](./credentials.mdx) — store upstream secrets and connect OAuth providers.
5. [**Authorization & Trust**](./authorization.mdx) — how rules, Permit policy, and human consent compose.
6. [**Connecting Agents**](./connecting-agents.mdx) — wire your agent runtime, Docker, or Kubernetes to the proxy.
7. [**Security**](./security.mdx) — SSRF protection, credential isolation, response scrubbing, TLS modes, agent identity and drift protection, and rate limits.
