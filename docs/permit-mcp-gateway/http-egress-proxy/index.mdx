...
| --- | --- |
| **Deny by default** | No outbound request is allowed until a workflow rule explicitly permits its host, path, and method. |
...
4. **Workflow match** — the request's host, path, and method are matched against the token's workflow. No matching rule means deny.
