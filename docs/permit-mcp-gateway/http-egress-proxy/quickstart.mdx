...
Enabling the proxy, storing credentials, and minting tokens can be done in the [dashboard](https://app.agent.security) or with the [`asg` CLI](./cli.mdx). **Workflows are authored in the dashboard** (there's no CLI for that yet), so this guide uses the dashboard for step 3 and the CLI — because it's copy-pasteable — for the rest.
...
- The proxy enabled for your account (see the availability note on the [Overview](./index.mdx)).
- The `asg` CLI installed (see [Install](./cli.mdx#install)) and pointed at your gateway with an admin token (see [Configure](./cli.mdx#configure)).
:::
...
The proxy injects it on the way out and [scrubs it](./security.mdx#response-scrubbing) from responses on the way back. For OAuth providers like GitHub, Google, Notion, and Atlassian, use [`asg proxy connect`](./credentials.mdx#oauth-connections) instead of a static key.

**Dashboard:** **CLIs / APIs → Credentials → Add credential**.

:::warning Storing production keys
Encryption at rest for credentials is opt-in (the KMS vault). Before you store a live key, confirm the vault is enabled on your gateway — see [Security → Credential isolation](./security.mdx#credential-isolation).
:::
...
HTTPS reaches the proxy as a `CONNECT` whose inner path and method aren't visible yet, so it can only match a rule with method class **All** and no path pattern. A `read` + `/v1/customers*` rule would deny every HTTPS call to the domain. You can add path- and method-scoped rules too — they apply to plain HTTP, and to HTTPS once you switch the rule to **intercept** (step 7). See [Workflows & Rules → HTTPS needs a tunnel rule](./egress-rules.mdx#https-needs-a-tunnel-rule).
:::
...
The `--out` path is the one [`asg proxy env`](./connecting-agents.mdx) expects, so the next step finds it automatically. The token only permits requests that match `charge_customers`. To instead let a **human** delegate access to an agent within a trust ceiling, use [`asg proxy authorize`](./authorization.mdx#human-consent) — that's the consent flow.
...
Use intercept where you need credential injection, path rules, or scrubbing on HTTPS; keep passthrough for domains where domain-level control is enough, or for very large uploads, downloads, and long-lived streams. See [HTTPS interception](./connecting-agents.mdx#https-interception) for the full picture. (Over plain HTTP, injection works with no intercept setup at all.)
...
`doctor` checks gateway reachability, whether the proxy is enabled, your stored-credential count, your local environment variables, the interception CA's status, your token file's permissions and expiry, and — when [agent identity](./security.mdx#agent-identity-and-drift-protection) is enabled — the agent's session registration and drift-block state. Each check reports **pass**, **warn**, or **fail** with a suggested fix.

---

## What's next

- [**Workflows & Rules**](./egress-rules.mdx) — refine workflows with domains, paths, methods, and trust levels.
- [**Credentials & Connections**](./credentials.mdx) — OAuth connections, injection methods, and AWS request signing.
- [**Authorization & Trust**](./authorization.mdx) — delegate access to agents through human consent and trust ceilings.
- [**Connecting Agents**](./connecting-agents.mdx) — Docker, Kubernetes, and HTTPS interception setup.
