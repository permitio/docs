...
| Proxy access token | A short-lived, workflow-bound token the agent presents to the proxy. Minted with [`asg proxy token create`](./quickstart.mdx#4-mint-a-workflow-bound-agent-token) or the [consent flow](./authorization.mdx#human-consent), and referenced via a token file. |
...
- When [agent identity](./security.mdx#agent-identity-and-drift-protection) is enabled: whether the agent has registered a session — and whether that session is **drift-blocked**, in which case the check fails with the re-approval link that clears it
...
- [**Quick Start**](./quickstart.mdx) — the end-to-end setup these commands fit into.
- [**Security**](./security.mdx) — what the proxy enforces on the traffic you route through it.
- [**The `asg` CLI**](./cli.mdx) — full command reference.
