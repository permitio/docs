...
To apply **full** governance to HTTPS — path-level rules, credential injection, and response scrubbing inside the encrypted connection — switch the relevant rule to **intercept** mode. You set a rule's TLS mode in the [dashboard](https://app.agent.security) workflow editor (**Proxy → Workflows**, edit the rule, set **TLS mode** to **Intercept**).
