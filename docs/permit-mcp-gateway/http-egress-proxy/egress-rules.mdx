...
| **Credential** | An optional stored [credential](./credentials.mdx) to inject into matched requests, so the agent never holds the secret. On an HTTPS domain, injection only happens on rules evaluated in `intercept` mode. |
| **TLS mode** | For HTTPS, whether the connection is `passthrough` (domain-level only) or `intercept` (decrypt and apply path/method rules in full). Read from the **tunnel rule** — see [Connecting Agents](./connecting-agents.mdx#https-interception). |

Because trust is required per rule, a single workflow can grant an agent broad read access while gating writes and deletes behind a higher trust level that only a human's consent can supply. See [Authorization & Trust](./authorization.mdx) for how trust is granted and capped.
...
4. **Bind an agent** — mint a token for the workflow with [`asg proxy token create --workflow <slug>`](./cli.mdx), or have a human delegate it with [`asg proxy authorize --workflow <slug>`](./authorization.mdx#human-consent).
...
- It requires the host to have **both** [agent identity](./security.mdx#agent-identity-and-drift-protection) and the guardian enabled.
...
- [**Credentials & Connections**](./credentials.mdx) — what a rule's attached credential injects.
- [**Authorization & Trust**](./authorization.mdx) — how a workflow's rules combine with Permit policy and human trust ceilings.
- [**Security**](./security.mdx) — SSRF protection and rate limiting that run alongside workflow matching.
