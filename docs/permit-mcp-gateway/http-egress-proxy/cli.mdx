...
Egress policy lives in **API workflows** (a set of allowed domains and rules). Today you create and edit workflows in the [dashboard](https://app.agent.security) under **Proxy → Workflows** — there is no CLI command for authoring them yet. The CLI handles everything around them: enabling the proxy, storing credentials, minting workflow-bound tokens, and connecting agents.
:::
...
| `asg proxy token create <host> --client-id <id> --workflow <slug>` | Mint a short-lived, workflow-bound agent proxy token (`--out <file>` writes it securely). |
| `asg proxy authorize <client_id> --workflow <slug>` | Run the browser consent flow so a human can delegate access to an agent for a workflow, within a trust ceiling. |
...
Run `asg proxy <command> --help` for the full flag list of any command. `asg proxy --help` also lists advanced commands for agent identity registration and the intent guardian; those are optional, opt-in capabilities — [contact us](mailto:support@permit.io) if you want them enabled for your account.
