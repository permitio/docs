...
The gateway creates **9 derived role rules per MCP server** in Permit to implement this calculation:

| Derived role on the server | Rules (role on `user_profile` + relation to server) |
| --- | --- |
| `{server}#high` (2 rules) | `owner` + `high`; `{server}-high` + `high` |
| `{server}#medium` (3 rules) | `owner` + `medium`; `{server}-medium` + `medium`; `{server}-high` + `medium` (high capped to medium) |
| `{server}#low` (4 rules) | `owner` + `low`; `{server}-low` + `low`; `{server}-medium` + `low` (medium capped to low); `{server}-high` + `low` (high capped to low) |

The `owner` rules give the human who owns the profile derived permissions on the server. The gateway admin dashboard uses them to discover a human's permissions.
