...
:::info
Every plan includes session expiry: a session ends 90 days after the last tool call, and each tool call resets that timer. Enterprise plans add configurable consent windows. See [Session expiry](/permit-mcp-gateway/consent-service#session-expiry).
:::
