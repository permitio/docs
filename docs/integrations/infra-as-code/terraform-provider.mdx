...
- `version = "~> 0.0.14"` accepts any `0.0.x` release from `0.0.14` on. For the latest release, see the [provider page in the Terraform Registry](https://registry.terraform.io/providers/permitio/permit-io/latest).
- Replace `YOUR_API_KEY` with your environment API key for a first local test. For anything you commit, leave `api_key` out of the file and set the `PERMITIO_API_KEY` environment variable instead. See [Pass the API key in environment variables](#environment-variables).
- `api_url` is optional and defaults to `https://api.permit.io`.
...
To use these variables, set `api_key = var.permit_api_key` and `api_url = var.permit_api_url` in the `provider "permitio"` block of `main.tf`.
...
The ReBAC example in the provider repository lists resource role permissions as action keys without the resource prefix, such as `["read", "write"]`. For the concepts, see [ReBAC policies](/how-to/build-policies/rebac/overview).
...
Use a data source to read an object that Terraform doesn't manage, such as a resource created in the dashboard. A `permitio_role` also requires `key` and `name`, which this excerpt leaves out:
