...
- We are using the [cloud PDP](/concepts/pdp/overview#hostedmanaged-cloud-pdp-option) which is hosted by Permit, but keep in mind that it does not support ABAC and RBAC policies. In production, you should [deploy your own PDP](/how-to/deploy/deploy-to-production).
