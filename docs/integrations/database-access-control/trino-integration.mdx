...
4. **Configure Trino to use the PDP for access control**
   - Configure the Trino's `access-control.properties` to call the PDP:
   ```yaml
...
Point Trino’s OPA plugin at the PDP endpoints by updating the `access-control.properties` file.

Path: `/etc/trino/conf.dist/access-control.properties`
