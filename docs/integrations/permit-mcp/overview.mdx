...
It can enable authorized individuals and end users to request and manage access to a resource using natural language. 
...
To set up the server, you to need supply the environment variables defined in the `.env.example` file. Create a `.env` file in the root directory and specify the following variables: 
...
**How it works**:
The CLI tool will allow parents and children to order dishes from various restaurants, with some restrictions. Children can only access certain restaurants or dishes if they've been granted permission.
...
- A user management element called "Restaurant requests" to enable parents manage requests sent by children to access a restricted restaurant.
...
We will be using the local Permit PDP for this project instead of the cloud PDP, as it enable implementation of ReBAC authorization which is not yet avaialble in the cloud PDP.
...
Then, import the necessary libraries, loads environment variables, determines the database name from command-line arguments, and establishes a connection to a SQLite database.
...
To keep this concise, we've omitted some utility functions--such as database initialization and MCP-to-Gemini tool conversion helpers. You can find these here in the [utils file](https://github.com/Tammibriggs/permit-mcp/blob/main/examples/food-ordering-system/utils.py). 
...
Finally, this block handles any errors and ensures the cleanup of the WebSocket and MCP connections.
