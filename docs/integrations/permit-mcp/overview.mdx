...
description: "Set up the Permit MCP server to create and manage access and approval requests through natural language, in Claude Desktop or a production app."
...
In this project we are using the local Permit PDP instead of Cloud PDP primarily for network and operational control reasons (running close to the app, fully within your own infrastructure).

For more details on when to use Cloud PDP vs. a local PDP, see [Cloud PDP Capabilities](/concepts/pdp/cloud-pdp-capabilities).
