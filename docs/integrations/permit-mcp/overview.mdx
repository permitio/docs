...
---
The Permit MCP server provides tools to create and manage access and approval requests for a resource. 
tt can enable authorized individuals and end users to request and manage access to a resource using natural language. 

This tutorial will guide you through setting up and using the server in Claude Desktop and also show how to use it in a production setup to build an AI-powered application for your users.
...
The server can be used to provide tools that allow end-users to send access requests from within an AI application. In this setup, the MCP server and the LLM run in a secure environment where user queries are processed. 
You can find an implementation of this in the [Family Food Ordering System](https://github.com/Tammibriggs/permit-mcp/tree/main/examples/food-ordering-system) example.
...
## Build a Family Food Ordering CLI Tool With the Permit MCP Server
...
**How it works**:
The CLI tool will allows parents and children to order dishes from various restaurants, with some restrictions. Children can only access certain restaurants or dishes if they've been granted permission.
...
## Creating a Custom MCP Server and  FastAPI Backend 
...
Next, specify the following environment variables. You can get the your Gemini API key from [Google AI Studio](https://aistudio.google.com/app/apikey). 
```python
PERMIT_PDP_URL= # The local Permit PDP URL http://localhost:7766
GEMINI_API_KEY=
DB_NAME=food_ordering.db
```
We will be using the local Permit PDP for this project instead of the cloud PDP as enables implementation ReBAC authorization which is not yet avaialble in the cloud PDP.


### Extend the Permit MCP Server With Additional Tools
To build a functional food ordering CLI, we need to augment the Permit MCP server with a few additional capabilities.

Currently, there is no way for family members to order dishes or even list the available dishes at a restaurant, 
even though they can view available restaurants by listing resource instances. To address this, we'll create a custom MCP server that includes the existing Permit tools along with two new tools:
...
Then, imports necessary libraries, loads environment variables, determines the database name from command-line arguments, and establishes a connection to a SQLite database.
...
Next, add the `list_dishes` tool which returns a list of dish names and their prices from a database if a user is permitted, otherwise, it raises an access denied error: 
...
In this section we will create a FastAPI backend were we will securely connect to the MCP server to access it's tools and pass them to the LLM. 
The backend is neccessary as we are working with some sensitive environment variables like our Permit API key and Gemini API key. 

The FastAPI backend will include a login endpoint to authenticate users, allowing us to control tool access based on user roles and pass relevant user-specific data to the LLM. 
It will also provide a WebSocket endpoint to handle user queries and return real-time responses.

To keep this concise, we've omitted some utility functions--sush as database initialization and MCP-to-Gemini tool conversion helpers. You can find these here in the [utils file](https://github.com/Tammibriggs/permit-mcp/blob/main/examples/food-ordering-system/utils.py). 
Create a copy of the file in the root directory of your project.

Now, let's get started with creating the FastAPI backend. 
In the root directory, create a `server.py` file. First, imports the neccessary libraries, sets parameters for connecting to the food ordering MCP server via stdio, initializes the GenAI client, and creates the FastAPI app with a startup event to initialize the database.
...
Next, create the login endpoint to authenticate users and generate a JWT access token using the user's identity which will be used for subsequent authenticated requests. 
...
Next, let's create the chat WebSocket endpoint which will connect to the MCP server, pass it's tool to the LLM and process user queries. Since it's a bit lengthy, we'll break it into parts.
...
Next, the MCP session is initialized, it's tools are then retrieved and filtered based on the user's role. The filtering ensures that children don't have access to tools to mange access or approval requests. 
...
This part executes any suggested function calls concurrently using the MCP server, with the results are added to the history.
...
Finally, this block handles any errors and ensures cleanup of the websocket and MCP connection.
...
You can find the complete code this [server.py file](https://github.com/Tammibriggs/permit-mcp/blob/main/examples/food-ordering-system/server.py).

## Building the Command-Line Chat Client
This client application will allows users to interact with the FastAPI backend via the command line. 
It will handle user login to obtain an authentication token and connects to the WebSocket endpoint to send messages and receive AI responses in real-time.

Create a `client.py` file in the root directly, and start by adding the following lines to import the necessary libraries and define the FastAPI server's base URLs.
...
When ready, it reads user input from the terminal using `input()` inside a thread executor (to avoid blocking the async loop), and if the input is `"exit"`, the chat ends.
Otherwise, it sends the user message and current chat history as a JSON payload over the WebSocket.
...
The `main` async function greets the user, prompts for username and password and calls the `login` function. If login is successful (a token is received), it then calls the `chat` function to start the WebSocket session. 
The `if __name__ == "__main__":` block is the standard entry point, running the main coroutine and catching `KeyboardInterrupt` for graceful exit.
```python
async def main():
...
Next, in another terminal instance, run the CLI using the following comamnd: 

```shell
uv run client.py
```

With this we can now login using one of the following credentials and start using our family food ordering system.
