...
Create a resource for each of your application's resources. E.G. if you have user management section in your app you will need the `user` resource with `create` `delete` and `get` actions, if you have a task management section you will need the `task` resource with `create` `delete` `get` and `accomplish` actions.
...
Check each action you want to allow for each resource for each role.
...
On Your backend side, you will need to add code that will check Auth0 JWT token's validity and get the user's information from it.

In our demo app we used Auth0 and NextJs integration to do it with very few lines of code as you can see in `[auth0].ts` and the getSession in `tasks.ts`.
...
### 2.2. Permit
As we want to use Permit for authorization, we will use the Permit SDK to get the user's permissions and check if they have the right permissions.
Authorization is only done on the backend side, so we will use the `permit` checks before each action, you can add it at your CRUD APIs or as a middleware for all the API.
...
In the demo app you can see it at `pages/api/tasks.ts` as a middleware that manage all task CRUD requests.
...
We will do it by adding a `postLogin` function that will be called after each login.

And there we will make sure that the user and roles are synced with Permit. you can check the [NextJS demo app for the full code.](http://localhost:3000/integrations/authentication/auth0-demo)
