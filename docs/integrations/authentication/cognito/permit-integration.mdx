...
## Introduction
In this tutorial, we will show you how to integrate Cognito with Permit.
...
Permit is a fully managed authorization service that allows you to manage your application's policies in one place.

Together, Cognito and Permit allow you to easily manage your application's authentication and authorization in a secure and scalable way.
...
- An application that uses Cognito for authentication; in this example, we will use a Vanilla JS webapp with Node server [Check out the demo explanation for more details](/integrations/authentication/cognito/cognito-demo-app).
...
E.G. many simple RBAC application will have 3 roles: `admin`, `editor`, and `viewer`.
...
## 2. Integrate your own application with Cognito and Permit
### 2.1. Connect Cognito to your frontend application


As we want to use Cognito for authentication, we will use the Cognito to get each user's information and access token.
...
After the user is logged in, we will need to sync their information with Permit.
We can tell Cognito to redirect us to as specific URL after the user is logged in or we can use the `code` query parameter they send after the login to sync the user to Permit.
...
On your backend side, you will need to add code that will check the validity of the Cognito JWT and retrieve user information from it and sync the user to Permit.
...
- `your permit token` - the token of your Permit account, [Get your environment API Key](tutorials/quickstart.mdx#get-your-permitio-environment-api-key)
...
Now revoke some of the permissions in permit dashboard and try to perform the same actions again, it will take effect immediately without refresh.
