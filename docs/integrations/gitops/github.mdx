...
This guide is demoing GitOps on Github, but should work in a similar fashion to other Git repositories which can be configured with an SSH key.
...
:::info Note
This deploy key shouldn't use 2 factor authentication. So Permit server can read and write to your repository with this key.
:::
