...
9. Replace the placeholder with your project id or key and execute a POST request to Permit with your json:
    ...
    ```
10. Go back to your GitHub repository, you should see new branches, one for each environment in your project. the branches are named after the environment id like so `permit/generated/<env_id>`.
...
to do this, replace the placeholder with your project id or key and execute a PUT request to Permit:
    ...
    ```
12. To adding custom policy code for each env, you need to checkout to the right branch `permit/generated/<env_id>`. You should only add you policy code inside the `custom` folder. You can either add your custom Rego code to the `custom/root.rego` file or add another file and import it in the `custom/root.rego` file (if you edit any policy code outside this folder it can be override by out code / break the rego code)
Once you edit the code inside any environment branch and push it to the remote GitHub repo the, Permit cloud will identify this change and pass it to the PDP that is connected to this environment should update.

13. Your git repository is now used for your PDPs policy 🚀