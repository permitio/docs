...
3. Copy the SSH url from GitHub:
    _![Copy SSH url](/img/integrations/gitops/github-copy-ssh-url.png)_
4. Copy the SSH private key you've generated [previously](#generate-an-ssh-key) and replace new lines with "\n".
    You can do the new line replacement using the following command:
    ```shell
    awk -v 'ORS="\\n"\ '1' <private-key-file>
    ```
5. Paste your values into that json and store at in a file:
    ```json
    {
      "url": "<your-ssh-url>",
    ...
    ```
6. Replace the placeholder with your project id or key and execute a POST request to Permit with your json:
    ```shell
    ...
    ```
7. Go back to your GitHub repository, you should see new branches, one for each environment in your project.
8. After validating your new branches policy, you need to activate the new repository in permit,
