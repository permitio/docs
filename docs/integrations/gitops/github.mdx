...
2. Press F12 to open the DevTools and extract your JWT token
3. Login to api.permit.io using Postman or curl
    ```shell
    curl -H "Authorization: Bearer <your-jwt-token>" -c permit_cookie.txt -X POST https://api.permit.io/v2/auth/login
    ```
4. Switch organization to your relevant organization id using
    ```shell
    curl -X POST "https://api.permit.io/v2/auth/switch_org/{org_id}" -H "Content-Type: application/json" -b permit_cookie.txt -c permit_cookie.txt"
    ```
...
8. Replace the placeholder with your project id or key and execute a POST request to Permit with your json:
    ```shell
    curl -X POST "https://api.permit.io/v2/projects/<your-project-id-or-key>/repos" -H "Content-Type: application/json" -b permit_cookie.txt --data-binary "@path/to/your/json"
    ```
