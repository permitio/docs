...
If you already have a repository you can skip to the [next step](#generate-an-ssh-key).
...
branches are created and mapped for each Permit environment. Additional layout is supported, but not currently available via the API.
...
2. Copy the public SSH key you have generated [previously](#generate-an-ssh-key) to your clipboard and paste it in the GitHub prompt.
