...
We provide you with a **Policy-Decision-Point** - aka an authorization microservice. It's available as a container for you to use,
or we can provide you with a cloud version for much faster adoption.

<Tabs groupId="pdp">
<TabItem value="cloud" label="Cloud PDP" default>

It is exremely simple to utilize the cloud PDP. As part of the initialization of the Permit instance, you need to pass
the cloud PDP URL.

:::note
Please remember that for the time being, when using the Cloud PDP, **there is no support** (**ABAC Attribute-based Access Control**).
:::
