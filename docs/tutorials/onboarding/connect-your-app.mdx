...
import PythonConnect from "@site/docs/sdk/python/role/get-role.mdx";
...
<TabItem value="Ruby" label={<RubyLabel/>} default>
    ```ruby
    permit.sync_user({ "key": user.id, "first_name": user.first_name, "last_name": user.last_name, "email": user.email})
    ```
</TabItem>
