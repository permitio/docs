...
To start the PDP disconnected, set both `PDP_ENABLE_OFFLINE_MODE` and `PDP_CONTROL_PLANE_CONNECTIVITY_DISABLED` to `true`. The PDP will load its configuration and policies entirely from local backup and will **not** attempt to connect to Permit Cloud on startup.
