...
That's useful when you want to have a single API Key for all your environments, or when you want to have a separate API Key for each of your projects.

To do that, you can set the `PDP_ORG_API_KEY` or `PDP_PROJECT_API_KEY` environment variable instead of `PDP_API_KEY`.
In addition, you'll need to set the `PDP_ACTIVE_ENV` and `PDP_ACTIVE_PROJECT` (only if you're using a Organization API Key) environment variables with the ID or Key of the environment and project you want to use.
