...
For extra resilience, the PDP can be configured to work in offline mode, serving decisions from a local backup when it can't reach Permit's cloud. Offline mode also supports starting the PDP fully disconnected from the control plane, with runtime API endpoints to toggle connectivity without restarting.

See the dedicated [PDP Offline Mode](/how-to/deploy/offline-mode) page for full details on setup, configuration, and the connectivity control API.
