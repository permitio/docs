...
An unhealthy PDP will return a 503 status code when The PDP failure rate is more than the configured threshold.
...
1. `PDP_OPA_CLIENT_FAILURE_THRESHOLD_PERCENTAGE` - The failure threshold percentage for the Policy Engine healthcheck. Default value is 10%.
2. `PDP_OPA_CLIENT_FAILURE_THRESHOLD_INTERVAL` - The interval (in seconds) to calculate the failure threshold. Default value is 60 seconds.

Default configuration will return 503 status code if at least 10% of the requests made to the PDP on the last 60 seconds failed.

### Readiness Probe

The readiness probe is used to determine if the PDP is ready to receive traffic,
it returns a 200 status code if the PDP is ready to receive traffic. The readiness probe endpoint is `/healthy`.<br />
An unready PDP will return a 503 status code if the Policy Engine latest policy update failed.

### Startup Probe

The startup probe is used to determine if the PDP is ready to receive traffic on startup,
it returns a 200 status code if the PDP finished the startup and is ready to receive traffic. The startup probe endpoint is `/ready`.<br />
A PDP will be considered ready and return 200 once the Policy Engine has finished to load the policy from the policy store ( Git repository ) successfully.
