...
Increase storage for PostgreSQL and for OpenSearch, which stores audit logs. The chart reads `persistence.size`. The chart doesn't set a storage class, so the volumes use the cluster's default storage class and `storageClass` has no effect. Kubernetes can only expand an existing volume when the storage class allows volume expansion.
...
Tune OpenSearch shards, replicas, and index rollover for the audit log index. The example shows the keys without their parent. In `values.yaml`, nest the `opensearch` block under `thirdPartyServices`, because the chart reads `thirdPartyServices.opensearch.indexSettings` and `thirdPartyServices.opensearch.ism`:
...
The installer keeps service passwords in `global-infrastructure-secret` and builds the database connection string in `permit-backend-v2-secret` from `values.yaml`. Every installer run reuses the passwords already in `global-infrastructure-secret`. The chart doesn't create a `postgres-secret` secret, so the first command in the block below applies only to clusters where you created that secret yourself. Patching a secret doesn't change the password of the PostgreSQL user in the database. For help planning a rotation, contact [Permit support](#support).
