...
- **On-premise**: k3s, k8s, kubeadm clusters
...
<div className="bg-green-50 border-l-4 border-green-400 p-4 my-4">
  <div className="flex">
    <div className="ml-3">
      <p className="text-sm text-green-700">
        <strong>Optional:</strong> Git repository setup is only required if you want to use Git-based policy management. You can set this up later through the UI.
      </p>
    </div>
  </div>
</div>
...
If configuring Git during installation, you'll need:
...
- **PostgreSQL database**: 20GB minimum, 50GB+ recommended
- **OpenSearch logs**: 10GB minimum, 100GB+ for production
...
- [ ] **Frontend domain planned** (what domain will customers use to access the platform)
- [ ] **Git repository ready** (optional - for policy sync)
- [ ] **SSH key configured** (optional - if using Git during installation)
