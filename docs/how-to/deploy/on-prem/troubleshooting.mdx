...
2. Test the frontend from inside the cluster, bypassing the ingress. The `permit-frontend` service listens on port `80` and forwards to container port `3000`. If the port-forward below fails, forward to the service port instead: `kubectl port-forward -n permit-platform svc/permit-frontend 3000:80`.
   ```bash
...
2. **The ingress doesn't route traffic.** The chart names the ingress `permit-platform-ingress`. Use that name in `kubectl describe ingress` if `permit-frontend` isn't found.
   ```bash
...
The installer stores the admin password in the `global-infrastructure-secret` secret under the `KEYCLOAK_ADMIN_PASSWORD` key. [Retrieve service credentials](/how-to/deploy/on-prem/management#retrieving-service-credentials) shows the command. The commands in this section read `keycloak-admin-secret`, where older installations keep the admin password.
...
3. **The password in the secret is wrong.** The installer stores the PostgreSQL password in `global-infrastructure-secret` under `POSTGRES_PASSWORD` (see [Retrieve service credentials](/how-to/deploy/on-prem/management#retrieving-service-credentials)). The chart doesn't create a `postgres-secret` secret. The command below works only in clusters where someone created that secret:
   ```bash
