...
1. **DNS issues:**
   ...
   echo "127.0.0.1 permit-platform.local" | sudo tee -a /etc/hosts
...
helm uninstall permit-platform -n permit-platform 2>/dev/null || true
helm uninstall third-party-services -n permit-platform 2>/dev/null || true  
helm uninstall migrations -n permit-platform 2>/dev/null || true
