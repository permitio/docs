...
The chart templates read `global.postgres`, `global.redis`, and `global.rabbitmq` connection settings, and `adminUsername`, `adminPassword`, and `db.password` under `global.keycloak`. The templates don't read `global.opensearch` or the `realm`, `clientId`, and `clientSecret` keys under `global.keycloak`, so changing those keys has no effect.
...
The chart reads `persistence.size` for PostgreSQL, Redis, OpenSearch, and RabbitMQ. The chart doesn't set a storage class, so the persistent volume claims use the cluster's default storage class, and `persistence.storageClass` has no effect. The Keycloak database claim, `keycloak-postgres-pvc`, requests a fixed 5Gi, so `thirdPartyServices.keycloak.persistence.size` has no effect.
