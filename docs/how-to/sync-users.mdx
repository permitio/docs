...
by using the supported [SKDs](/how-to/sync-users#via-the-sdks), or via the [API](/how-to/sync-users#via-the-api).
...
New users should be synced into Permit as part of the sign-in/sign-up process, or, in other words, as soon as the authentication solution verifies their identity.
This is also the point at which you should translate a users **organizational role** (I.e - Head of Marketing / Costumer / Consultant / Patient) and any other
...
Generally, creating users via the SDK requires the use of a single function - `syncUser`.
...
Once assigned with a role, a user will be ablt to perform actions that are permitted for that role.
...
We [integrate with any authentication solution](/integrations/authentication/your-authentication) and provide the `syncUser` function for all of our SDKs.

A detailed explanation of how to use the `syncUser` function with each of our SDKs [is available here](/category/supported-sdks). The `syncUser` function will consist of:
...
seperatly via the [`Assign Role To User`](https://api.permit.io/v2/redoc#tag/Users/operation/assign_role_to_user) function.

A full list of all API functions, their uses and purpouse [is available here](https://api.permit.io/v2/redoc#tag/Users).
