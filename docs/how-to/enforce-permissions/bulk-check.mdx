...
The most basic usage of a `bulkCheck` is to save time when multiple checks calls are needed in one scope.
For example, assuming we have an API endpoint that perform multiple actions on a resource, we can use the `bulkCheck` function to check all the permissions at once.
...
For example, if `edit` is a premium feature in our application (with ABAC) and we also wants to check if the user has access to `edit` a document (with ReBAC), we can use the `bulkCheck` function to check both permissions at once.
...
:::note
While `bulkCheck` will save you time in latency, it could have degraded performance if you're checking permissions for different tenants.
This is because we split the requests by tenant in order for the request to reach the relevant PDP instance. This is done for it to be compatible with the current [PDP Sharding](/how-to/manage-data/pdp-sharding) setup, in which that instance only contains data for a few tenants, not all of them.
:::
