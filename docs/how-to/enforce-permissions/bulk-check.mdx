...
providing requests on different tenants. This is because we split the requests by tenant in order for the request
to reach the relevant PDP instance, we do that to be compatible with the current [PDP Sharding](/how-to/manage-data/pdp-sharding)
setup, in which and that instance only contains data for a few tenants, not all of them.
