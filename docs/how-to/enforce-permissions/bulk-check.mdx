...
sidebar_position: 2
title: Bulk Check
description: "Validate multiple permission requests in a single call with Permit's bulkCheck() function."
---
...
While there are more efficient ways to perform data filtering with Permit, mapping multiple permissions checks to a single `bulkCheck` call can be useful in some cases.
