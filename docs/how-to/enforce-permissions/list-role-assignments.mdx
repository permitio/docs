...
To get the list of role assignments from the local PDP, you can use the `permit.local.role_assignments.list` function.<br />
This function accepts optional `user`, `tenant`, `resource`, `role`, `resource_instance` filters,
...
- `tenant` - The tenant to which the role is assigned,
    in case of resource role, this represents the tenant that the resource instance is associated with.
- `role` - The role assigned to the user.
- `resource_instance` ( optional ) - The resource instance to which the role is assigned.
