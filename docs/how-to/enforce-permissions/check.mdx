...
sidebar_position: 1
title: Permit.check()
description: "Call permit.check() to enforce RBAC, ABAC, or ReBAC permissions with tenants, attributes, and context objects."
---
...
* **Context** *(optional)* - a context object that can be used to pass additional data to [custom policy](/integrations/gitops/custom_policy/).
...
- A unique `userId` (e.g. the identity from your authentication - `jwt._subId`)
...
- Another example, access a document with id `doc1.txt`: 
