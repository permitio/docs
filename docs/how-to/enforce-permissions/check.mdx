...
* **Action** - a `string` representing action the user is attempting to perform.
* **Resource** - a `string` or `object` representing the resource the user is attempting to perform the action on. Resource could point to a resource type or a to a specific resource instance for fine-grained decisions.
...
:::tip
To maintain least code changes in your application, you should choose carefully the most granular `permit.check()` function implementation that fits your use case.
:::

The follwing examples demonstrates common `permit.check()` examples depend on the policy you are checking against, and the required level of granularity:
...
:::info
While all the examples in this page are written in JavaScript (Node.js SDK), permit.check() function exists in all Permit SDKs.
:::
