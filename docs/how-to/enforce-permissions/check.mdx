...
Depending on the complexity of the policy being enforced, `permit.check()` accepts several parameters to returan a boolean answer - `Permitted=True` or `Permitted=Flase`.

There are many ways to use the function within permit to enforce permissions. This can depend on the policy
...
:::tip Simple example
Say you have two tenents - `Tenant A` and `Tenant B`. The same user can be part of both tenants, with a different role in each one.
...
There are limits on overall data volume that can be loaded to a single PDP, which can be bypassed via [PDP Sharding](https://docs.permit.io/how-to/manage-data/pdp-sharding/).
:::
...
:::tip
In the code example below, we are checking if `John` has the `assign` permissions on a member group resource (Defined in Permit beforehand).
:::
