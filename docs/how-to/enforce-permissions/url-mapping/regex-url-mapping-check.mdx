...
description: "Map API endpoints to resources and actions using regex URL patterns, then check permissions with the PDP's allowed_url endpoint."
---
...
Notice in the API call, the regex pattern is being sent as a JSON string. In JSON, backslashes need to be escaped themselves, so `\.` becomes `\\.` in the JSON payload.
...
:::note 
The `secret` value is still required for now, even though it isn't used. This will be fixed in a future release of the proxy_configs API.
:::
