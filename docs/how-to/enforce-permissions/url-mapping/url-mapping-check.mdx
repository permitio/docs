...
description: "Map HTTP URLs to resources and actions using simple path parameters, then check permissions through the PDP."
---
...
This feature is available through the URL Mapping UI & PDP. You can access the URL Mapping UI inside the Permit App [here](https://app.permit.io/url-mapping).
...
In the URL Mapping UI, you can set variables as part of the URL using the following syntax: `{var1}`. This feature is called templating the URL, and it's beneficial to reduce the overall number of mapping rules required.
...
The mapping rules feature in the URL Mapping UI links HTTP URLs to particular resources and actions within the Permit.io platform. These rules are essential for controlling access and permissions on your website or application.

![Mapping Rules Example](/img/url-mapping/mapping-rules.png)
