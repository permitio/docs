...
2. **Data Filtering from Queries**: This has four subtypes:
   ...
   - **Application-Level Filtering**: For example, using a set of boolean results obtained from the Policy Decision Point (PDP) through [bulk checks](/how-to/enforce-permissions/bulk-check).
   ...
   - **PDP-Level Filtering Based on the Information Graph**: This involves returning pre-filtered objects based on data synchronized with the PDP in advance. It includes functions like [`getUserPermissions`](/how-to/enforce-permissions/user-permissions) (which returns all objects a user has access to) and [`getAuthorizedUsers`](/how-to/enforce-permissions/authorized-users) (which returns all users who have access to a specific resource).
