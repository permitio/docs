...
The following check asks whether a user can perform the `edit` action on one file. The code builds the instance key at runtime:
...
The script prints `true` and then `false`. These checks confirm the policy:
