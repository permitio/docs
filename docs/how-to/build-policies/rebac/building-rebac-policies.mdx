...
description: "Build a ReBAC policy in the Permit policy editor by defining resources, relations, role derivations, and instance relationships."
---
...
Relations are later instantiated between specific resource-instances as relationships (also known as relationship-tuples) see below.
...
Say you want to place a specific `document` under a specific `folder`, or a specific `account` under a specific `company` - you can do so via the Permit `Directory` under the `instances` tab.
...
```
And we can also pass, as in RBAC, an object with additional data such as the tenant:
```javascript
...
## Dive deeper into ReBAC with our example apps
  ...
  - [Galactic Healthcare](/modeling/rebac-GHC) - Healthcare app with user profile access delegation
  - [Pink Mobile](/modeling/pink-mobile) - Teleco representative app - with account level access
