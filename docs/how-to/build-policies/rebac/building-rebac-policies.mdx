...
Each node in the graph represents a resource, while edges depict the relationships between them, such as ownership or permissions.
Initially, the graph is read-only, allowing users to explore the structure and understand how resources are linked without the risk of making changes.
This view helps provide clear insights into the resource hierarchy and relationships at a glance.

Here is the button to the graph.
