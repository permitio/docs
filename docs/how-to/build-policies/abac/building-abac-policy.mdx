...
Dynamic role are groups of users that share common attribute values. Think of them as dynamic groups that automatically include users based on their attributes.
...
Dynamic Resource group resources based on their attributes. In our example, we need to identify `Bicycles available after 5pm`
