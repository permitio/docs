...
In some cases, you'd like to create conditions that compare resource and user attributes, such as comparing a resource and user risk score. This comparison is available in Permit using the `(ref)` operators. Here's an example of such a condition:
