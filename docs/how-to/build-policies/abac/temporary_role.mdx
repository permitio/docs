...
- ### Configure time-based groups
  ...
  ```
:::info
The `expires` value above is the Unix timestamp in milliseconds. This requires that the `current_time` value is passed in the `permit.check()` as a user attribute.
:::
