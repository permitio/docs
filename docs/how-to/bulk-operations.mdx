...
description: "Perform bulk operations on users, tenants, roles, role assignments, and relationship tuples through the Permit API."
---
...
Bulk calls support batching multiple items in a single request.  
The following limits apply per call:

| call                  | Maximum Items per Call |
| ---------------------- | ---------------------- |
| Users                 | 3,000                  |
| Tenants               | 2,000                  |
...
Bulk calls are also subject to additional [rate limits](/api/api-with-cli/#rate-limiting).  
Bulk APIs are not intended for full or large-volume data migrations; please consult with our support team before performing large-scale data migrations.
