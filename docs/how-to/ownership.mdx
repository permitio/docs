...
description: "Model resource ownership in Permit using either an ABAC owner attribute or a ReBAC parent-child relationship."
...
While offering a very granular way of managing ownership on a per-resource level, ABAC lacks the ability to rely on existing application-level relationships (Such as a file residing within a folder).
...
5. In the `Direcrory` section, click on `Add User`. Create a new user who you want to give access to the specific file instance.
   In our case, we created a user named `Bob` and gave him the resource role `Bobs_Files#owner`.
...
7. Going back to the `Policy Editor` screen, you will be able to see the `file#owner` role and the `folder#owner` role.
   As `file` is set as parent of `folder`, all permissions set on a `folder` will automatically propagate to `files` within those folders as well.
