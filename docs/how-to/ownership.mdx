...
:::tipExample Policy
For the purpouse of this example, let's try and setup the following policy:
...
While providing the obvious benefit of managing hierarchical ownership scearios, ReBAC might be a little more complex to model.

### Setting up ReBAC based Ownership with Permit

:::tipExample Policy
For the purpouse of this example, let's use the same one from before:
`Bob` can `Read`, `Upadte` and `Delete` every file he is an `Owner` of.  
:::
...
3. In the `Policy` section, got to the `Resources` tab. Click on `Add Attributes`.
...
5. Going back to the `Policy Editor` screen, you will be able to see the `file#owner` role and the `folder#owner` role.
   As `file` is set as parent of `folder`, all permissions set on a `folder` will automaticaly propogate to `files` within those folders as well.
