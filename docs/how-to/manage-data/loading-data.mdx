...
Keeping the authorization data synchronized with external systems is essential for maintaining accurate access control. Event-driven data updates enable real-time synchronization between external systems and authorization platforms like Permit.

Using Permit's APIs, we can update date to Permit in real-time based on the events from external systems. Apart from APIs, we can also use OPAL scope, PDP updates, etc, to achieve the same. This allows you to update the data in real-time.

What can be some of the use cases for this?

- **GitHub Organization Membership Sync:** A company can update their GitHub org membership data automatically and have it synced to Permit. By syncing their org membership data in real-time, companies can enforce policies based on these memberships using GitHub Webhooks.

- **Database User Permissions Sync:** A company wants to enforce policies based on the user permissions in the database. The company can use the database triggers to sync the data to Permit. For instance, when a new user is added to the database, their permissions will be synced to Permit automatically.

To learn more about how to sync data to Permit, check out [this detailed guide](/how-to/manage-data/event-driven-updates) where we taken an example of syncing the GitHub organization membership data to Permit in real-time utilizing GitHub Webhooks.
