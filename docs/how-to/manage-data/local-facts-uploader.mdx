...
description: "Route data writes through the PDP with proxy_facts_via_pdp so permission checks see newly created data immediately."
---
...
**Recommended**: Deploy PDPs as a [Centralized PDP](/concepts/pdp/overview#centralized-edge-pdp) or as a [PDP Sidecar](/concepts/pdp/overview#hosted-as-an-edge-pdp-sidecar) to your application. This ensures the same PDP instance handles both data creation and permission checks.

**Less Optimal**: Using a [PDP Cluster](/concepts/pdp/overview#cluster-of-edge-pdps) with load balancing, permission checks may be routed to PDPs that haven't received the data update yet.
