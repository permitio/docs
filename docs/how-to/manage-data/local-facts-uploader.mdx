...
In the scenario where you need to check permissions immediately after creating or syncing a user there is sometimes a data sync latency of the data to the PDP. To help reduce the data sync latency before checking permissions, we've provided a mechanism that sends the data updates directly to the PDP first, and then propagates the data out via OPAL (including other PDPs if you operate in multi-PDP setup).
