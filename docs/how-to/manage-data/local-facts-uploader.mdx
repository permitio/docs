...
To help reduce the sync latency between Permit cloud and the PDP,
the PDP can be used for some of the APIs.
...
Waiting for the facts is available only when proxy_facts_via_pdp is enabled
...
async with permit.synced_facts() as p:
