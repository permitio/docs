...
To add user attributes, you can use the following endpoint that `Creates a User`. Replace `{proj_id}` and `{env_id}` with your
...
As part of the data, you can pass in `attributes` for that particular unique user identity. Here’s an example using cURL to
