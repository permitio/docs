...
Once the relationship is defined, save the configuration. You now have a clear hierarchical relation where Dashboard
...
graph TB
    %% Define Styles
    classDef resource fill:#f9f,stroke:#333,stroke-width:2px;
    classDef user fill:#cff,stroke:#333,stroke-width:2px;
    classDef role fill:#bbf,stroke:#333,stroke-width:2px;

    %% Users
    John[John Smith]:::user
    Anna[Anna Smith]:::user

    %% Resource Instances
    Dashboard1[Dashboard 1]:::resource
    WidgetA[Widget A]:::resource
    WidgetB[Widget B]:::resource

    %% Relationships
    John -->|Owner| Dashboard1
    Anna -->|Viewer| Dashboard1

    Dashboard1 --> WidgetA
    Dashboard1 --> WidgetB

    WidgetA -->|Analyst/Editor| Anna
    WidgetB -->|Viewer| John
```
