...
1. Alice is the **author** of Blog Post 1 and Blog Post 2. She has full **edit** and **read** permissions for these posts.
2. Bob is the **author** of Blog Post 3, with full **edit** and **read** permissions for that post.
3. Alice and Bob can both view each other’s blog posts but cannot edit them.
...
While a single `check()` call can determine whether Alice or Bob has permission to perform one action on one blog post, more
advanced use cases like filtering multiple blog posts or listing authorized users require **bulk checks** and other advanced
queries.
