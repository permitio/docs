...
- If you're not sure where to start, it’s best to opt for simpler models such as [Multi-Tenant](https://docs.permit.io/concepts/multitenancy/) [RBAC](https://docs.permit.io/how-to/build-policies/rbac/overview) or [ReBAC](https://docs.permit.io/how-to/build-policies/rebac/overview) with a simple hierarchy/direct [ownership](https://docs.permit.io/how-to/ownership).
...
- [Projects](https://docs.permit.io/manage-your-account/projects-and-env#projects) and [Environments](https://docs.permit.io/manage-your-account/projects-and-env#environments) within Permit can help you **silo your deployments effectively**.
...
- Leverage [Tenants](https://docs.permit.io/concepts/multitenancy/#tenants-in-permitio) to isolate users and **resources that should only work together**. This feature facilitates application-level multi-tenancy.
...
- [PDPs](https://docs.permit.io/overview/how-does-it-work#the-policy-decision-point-pdp), the network nodes responsible for answering authorization queries using policies and contextual data, are usually initially deployed as sidecars or within load-balanced containers using services like Fargate, Cloud Run, or Kubernetes (e.g., EKS).
- It’s important to ensure each PDP is correctly siloed to its intended environment using its [unique secret key](https://docs.permit.io/manage-your-account/workspace-settings#api-keys).

## Managing Policy SDLC and CI/CD

### SDLC

- You can implement [environment-specific access control](https://docs.permit.io/manage-your-account/workspace-settings#member-management) to manage **who can make policy changes** across different stages of deployment. For instance, a product manager may alter policies in the staging environment but would need a developer or DevOps to apply those changes to production.
...
- Consider using the [Permit Terraform provider](https://docs.permit.io/integrations/infra-as-code/terraform-provider/) to manage environment baselines and policy changes systematically.
- [Integration with Git](https://docs.permit.io/integrations/gitops/overview) allows merging and enforcing policy changes by managing authorization policies just as you would manage code.

## The Right Interfaces for the Right People

- [Policy editor access](https://docs.permit.io/manage-your-account/workspace-settings#member-management) can be allocated to stakeholders based on their role and technical proficiency. This ensures that non-technical staff and developers with limited requirements can make necessary adjustments without complications.
...
- [Permit’s embeddable UI components](https://docs.permit.io/embeddable-uis/overview) can be used to delegate access control functionality to your end users safely.
