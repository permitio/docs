...
each shard will have only the data of the tenants associated with him.
...
When a data changes on Permit.io it will be propagated to the specific PDP shard that is responsible for it,
same consistent hashing algorithm will be used to map the data to the correct shard.
...
Currently, we assume a fixed number of PDPs, although scaling would be available soon.

For the SDK’s queries to reach the correct PDP instance (holding the shard that has the correct tenant id), we are using Envoy.
...
Our backend sharding is implemented using that same algorithm, Thus both Envoy and our backend are mapping the same tenant ids to the same shard ids.


## Quickstart

An Envoy container with a custom configuration should be deployed next to PDPs.
    ...
    static_resources:
      ...
      - name: pdp_cluster
        ...
        load_assignment:
          ...
          endpoints:
          - lb_endpoints:
         # Replicate for each Shard / PDP {
            - endpoint:
                address:
                  socket_address:
                    address: $PDP-1-ADDRESS
                    port_value: 80
              metadata:
                filter_metadata:
                  envoy.lb:
                    hash_key: 1
         # }. And another one as an example:
            - endpoint:
                address:
                  socket_address:
                    address: $PDP-2-ADDRESS
                    port_value: 80
              metadata:
                filter_metadata:
                  envoy.lb:
...
3. Run Envoy:
    ```sh
   docker run --rm -it --network=envoy \
      -v $(pwd)/my-custom-envoy.yaml:/envoy-custom.yaml \
      -p 9901:9901 \
      -p 10000:10000 \
      envoyproxy/envoy:dev-d266941360e811eb34fc632d914211437e13f962 \
          -c /envoy-custom.yaml
    ```
4. Run PDPs:
...
**Note:** Sharded PDP is currently available for early access customers, make sure to contact us to get access to it.
