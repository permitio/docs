...
Currently, we assume a fixed number of PDPs, although dynamic scaling would be available soon.
...
Permit.io's backend uses that very same sharding algorithm, Thus both Envoy and the backend are mapping the same tenant ids to the same shard ids.
