...
description: "Meet the storage, memory, port, probe, and shutdown requirements for running Permit Nexus PDP, and learn its logging and security model."
sidebar_position: 5
---

This page lists what you need to run Permit Nexus PDP (`permitio/pdp-v3`), a self-hosted policy decision point (PDP), in production: storage, memory, ports, health probes, and shutdown settings. It also covers Nexus PDP logs and the Nexus PDP security model. It is for operators who deploy Nexus PDP on Kubernetes or another container platform. For what Nexus PDP is, see [Permit Nexus PDP](/concepts/pdp/nexus-pdp).
...
1. Send `GET /health/ready` to port `7001`, for example `curl -i http://localhost:7001/health/ready`. Nexus PDP returns HTTP `200` when every component that gates readiness is up, and HTTP `503` while any of them is still starting.
2. Point an SDK at port `7000` of the Nexus PDP and run a permission check. See [Check permissions](/how-to/enforce-permissions/check). A decision that matches your policy confirms that Nexus PDP has your policy and data.
...
:::warning Debug log levels write authorization data to Nexus PDP logs
`EDGE_NATS_LOG_LEVEL=trace` logs raw message payloads, which contain your authorization data. `EDGE_OPA_LOG_LEVEL=debug` logs authorization request and response bodies. Set these levels only for short debugging sessions, and remove them before the container serves production traffic. See [Nexus PDP child process logging](/concepts/pdp/nexus-pdp-configuration#child-process-logging).
:::
