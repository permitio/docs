...
[`permitio/pdp-v2` container PDP](/concepts/pdp/overview#run-a-local-pdp-with-docker).

## Supported Policy Models

- **RBAC (Role‑Based Access Control)** – **Supported**
- **ReBAC (Relationship‑Based Access Control)** – **Supported**
- **ABAC (Attribute‑Based Access Control)** – **Not supported**
...
| ---                                          | ---                                     | ---                                            |
| Deployment                                   | Managed by Permit                       | You run the Docker image                       |
...
| Debug mode configuration                     | Environment‑level (UI / API)           | Env‑level + `PDP_DEBUG` env var                |
| Logs forwarder / Fluent Bit                  | ❌ Not supported                        | ✅ Helm value `pdp.logs_forwarder.*`           |
| Metrics & APM                                | Internal only                           | ✅ Exposed (metrics, Datadog, etc.)           |
| Offline mode / backups                       | Managed by Permit                       | ✅ `PDP_ENABLE_OFFLINE_MODE` and backup volume |
