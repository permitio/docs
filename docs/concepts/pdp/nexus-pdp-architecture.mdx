...
What runs inside the [Nexus PDP](./nexus-pdp) container, which component is responsible for what, and the path a single authorization request takes.
...
They meet at the embedded database, which is why a control-plane outage costs freshness but never availability, and why a rebuild can run underneath a PDP that is still answering. See [How It Works](./nexus-pdp-how-it-works) for what happens along the sync path.
