...
description: What runs inside the Nexus PDP container, which component is responsible for what, and the path a single authorization request takes.
sidebar_position: 2
---

# Nexus PDP Architecture

What runs inside the [Nexus PDP](./nexus-pdp.mdx) container, which component is responsible for what, and the path a single authorization request takes.
...
![Architecture of the Nexus PDP: your app and orchestrator on the left, the single Nexus PDP container holding the authorization API, OPA, the health server, the query and bundle loopbacks, the embedded database, the change-ingest worker and the NATS leaf, with Permit.io and your policy store on the right](/images/pdp/nexus-pdp-architecture.svg)
...
Every hop is loopback or local disk. **No step in answering an authorization query contacts Permit's API**, by construction — the Nexus PDP has no control plane other than its event stream, and cannot reach `api.permit.io` for a decision even if it wanted to.
...
They meet at the embedded database, which is why a control-plane outage costs freshness but never availability, and why a rebuild can run underneath a PDP that is still answering. See [How It Works](./nexus-pdp-how-it-works.mdx) for what happens along the sync path.
