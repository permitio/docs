...
The architectural consequence is that **memory stops scaling linearly with your data set**. On the container PDP, a large environment needs a large PDP, and very large data sets need sharding. On the New Edge PDP, the data set lives on disk and memory is governed by the database's cache size, which you control.
...
- [New Edge PDP](./new-edge-pdp) — overview and architecture
