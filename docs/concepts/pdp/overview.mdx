...
We recommend deploying a local PDP for production deployments to minimize network latency and ensure high availability.
:::

### Run a Local PDP With Docker
To test your ReBAC and ABAC policies, you must run a local container of the PDP in your environment. Running the PDP requires a [local Docker installation](https://docs.docker.com/get-docker/).
