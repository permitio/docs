...
sidebar_position: 1
title: Overview
description: "What the Permit.io policy decision point (PDP) is, how the managed Cloud PDP, self-hosted Edge PDPs, and Nexus PDP differ, and how to deploy and run them."
---
...
Every permission check your application makes goes to a PDP, so PDPs need to be highly available and close to the services that query them.

The Permit PDP is your authorization microservice. It bundles OPA, the OPAL client, and an API server.

You run the PDP as a Docker container. Deployed next to your services, it answers checks locally, without a network round trip to Permit. As a sidecar on the same host, checks go over the loopback interface, so they carry no network latency and stay in the sub-millisecond range.
Like all of Permit's customer-deployed components, the [Permit PDP is open source](https://github.com/permitio/PDP) and available from [Docker Hub](https://hub.docker.com/r/permitio/pdp-v2).
...
- **[Nexus PDP](#nexus-pdp)** – A self-hosted PDP (`permitio/pdp-v3`) with an embedded on-disk database, kept in sync over a durable event stream.  
...
The [**Permit Nexus PDP**](/concepts/pdp/nexus-pdp) is a ground-up rewrite of the PDP runtime. You run one container per Permit environment in your own network. It keeps a local, on-disk copy of that environment's policy and authorization data and answers checks with no network call in the decision path. [Product overview](https://www.permit.io/nexus-pdp)
...
**When to choose it:** use Nexus PDP for large data sets and relationship-heavy authorization that you want to run in your own network. Use the container PDP when you need ABAC, custom Rego, read-your-own-writes, or the other capabilities listed on the [feature parity page](/concepts/pdp/nexus-pdp-feature-parity). Both connect to the same Permit environment and enforce the same policies, so you can run them side by side.

:::info Early access, with parity in progress
Nexus PDP is in early access and enabled per account; [contact us](mailto:support@permit.io) to turn it on. Feature parity with the Classic PDP (the container PDP) is in progress and ships continually.
:::

Read more: [Architecture](/concepts/pdp/nexus-pdp-architecture) · [How it works](/concepts/pdp/nexus-pdp-how-it-works) · [Feature parity](/concepts/pdp/nexus-pdp-feature-parity) · [Deployment](/concepts/pdp/nexus-pdp-deployment) · [Configuration](/concepts/pdp/nexus-pdp-configuration)
...
For more information, see the [sidecar deployment guide](/how-to/deploy/overview#sidecar-edge-pdp).
