...
:::info
We recommend you to migrate all your PDPs to FactDB.
:::

## FactDB Architecture

When the PDP boots, the FactDB download the latest snapshot from the Permit Cloud and stores it in the `/app/backup/` directory.
After the file is downloaded, the FactDB will resume data updates from the Permit Cloud since the given snapshot.
...
Using the stored data, when the PDP receive a query, it first checks the FactDB for the relevant data, then passes it to the OPA engine for policy evaluation.
