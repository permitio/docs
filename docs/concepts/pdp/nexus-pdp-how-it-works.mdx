...
How the [Nexus PDP](./nexus-pdp) keeps its local copy of your policy and authorization data current, what consistency that gives you, and what it costs to run.
...
A smaller footprint is possible by tuning the storage engine's cache and write-buffer sizes — see [Storage engine](./nexus-pdp-configuration#storage-engine). Benchmark against your own data set before committing to a size.
:::
