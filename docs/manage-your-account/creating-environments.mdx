...
This way, you can manage changes to your policy like code changes in your CI/CD pipelines.
...
Read more in the API Documentation about [Copying Environments](https://api.permit.io/v2/redoc#tag/Environments/operation/copy_environment).
...
Copy and merge environments applies only to **Policy** objects, such as Resources, Roles, and Condition Sets.
...
When we merge the Development environment back into the Production environment, we have a conflict with the `Admin` Role.

When merging environments, you can choose to resolve conflicts by either:
* **Fail**: (default) Fail the merge, rollback the entire merge operation and keep the existing object on the target environment.
...
Using wildcard is supported for `exclude` and `include` scopes to match multiple objects.
...
Now in your custom Git Repository, you will see a new branch with the name `new-env-branch-name` that corresponds to the new environment.
