...
Let's see how you can use the [Policy Guard API](https://api.permit.io/v2/redoc) for the example above.

:::note Info
For more information on Policy Guards, see the [Policy Guard documentation](/manage-your-account/policy-guard/policy_guard).
:::
...
```
:::note Info
We use `key` to refer to the resource, role, and action in the examples above. The `key` is a unique identifier for each object, enabling its reference across different projects within the Policy Guard scope.
:::
