...
4. Run `permit.check()` for a user and resource whose attributes make the condition true, and confirm the check returns `true`. Then change one attribute so the condition is false, and confirm the check returns `false`. See [Check permissions](/how-to/enforce-permissions/check).
