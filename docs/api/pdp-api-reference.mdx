---
sidebar_position: 2
title: PDP API Reference
---

# PDP API Reference

## PDP/SDK OpenAPI spec - overview and evaluation purpose of online version

You can access the spec at [https://pdp-api.permit.io/openapi.json](https://pdp-api.permit.io/openapi.json) which is embedded here:

<iframe
	style={{
		boxShadow: "0 1px 3px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.24)",
		backgroundColor: "white",
		maxWidth: "80vw",
	}}
	src={"https://pdp-api.permit.io/openapi.json"}
	width={"1080px"}
	height={"600px"}
/>


And view the redoc at [https://pdp-api.permit.io/redoc](https://pdp-api.permit.io/redoc) which is embedded here:

<iframe
	style={{
		boxShadow: "0 1px 3px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.24)",
		backgroundColor: "white",
		maxWidth: "80vw",
	}}
	src={"https://pdp-api.permit.io/redoc"}
	width={"1080px"}
	height={"600px"}
/>

## PDP/SDK OpenAPI spec - local version

Just like the Permit.io cloud service, the local PDP exposes an OpenAPI spec which can be used to develop your own SDKs.
Assuming you're running your PDP at `localhost:7000` you can access the spec at `http://localhost:7000/openapi.json` and view the docs at `http://localhost:7000/redoc`.

## Permit API object hierarchy

![Object diagram diagram](/img/diagrams/api-model.svg)