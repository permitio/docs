...
4. Send the request. Postman shows `200 OK` and the same JSON body as the curl command in [Call the API with curl](#using-curl).
...
3. Send many changes in fewer requests instead of one request per change. See [Bulk operations](/how-to/bulk-operations).
...
The API log covers calls to `api.permit.io` only. A permission check goes to a policy decision point (PDP) and appears in the audit log instead. See [Audit log types and filtering](/how-to/use-audit-logs/types-and-filtering#types-of-audit-logs).
