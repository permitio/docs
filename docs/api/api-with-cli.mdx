...
1. Go to the `API Log` tab under `Settings`.

#Add Screenhot#

2. This screen will display every API call made within the current organization.

3. The API Log screen consists of a Timestamp of when the API call was performed, the Actor who performed the call, the Action (The endpoint that was called), and the Response returned. 

#Add Screenhot#

4. To review a specific call, click on it, and it will display an `API Log Event`. 

5. The `API Log Event` includes additional information about the call, including `Request` and `Response` tabs. 

#Add Screenhot#

6. The `Request` tab includes the payload sent to the API endpoint, and the `Response` tab includes all the data received back from the endpoint. 
