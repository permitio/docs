...
In every command, replace `API_SECRET_KEY` with your API key, and `{project_id}`, `{proj_id}`, and `{env_id}` with your project and environment keys or IDs. The samples use the tenants `default` and `business`. Use one tenant key of your own in every call.
...
Send a PUT request to `/groups/{group_instance_key}/assign_group`. The body holds the key of the other group in the `type:instance` format, here `group:org1`. In the path, replace `{group_instance_key}` with `group:support`.
