...
In every command, replace `API_SECRET_KEY` with your API key, and `{proj_id}`, `{proj_id}`, and `{env_id}` with your project and environment keys or IDs. The samples use the tenant `default`. Use one tenant key of your own in every call.
