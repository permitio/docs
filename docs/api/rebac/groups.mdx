...
Let's say we want to represent a company and its employees as a group, and then assign all the employees access to a shared marketing resource (social-media) via the group.  We'll have a group type named 'company' (Which is represented as a resource named 'company'), and an instance of a specific company 'Acme'.  Users are assigned to `Acme` via a role called `member`
Additionally, we will have a resource-instance of type 'marketing' - 'social_media' which supports the role 'editor'.
Once we assign the role 'editor' to  the `Acme` group, the following will by automated by API:
...
```bash
curl 'https://api.permit.io/v2/facts/{project_id}/{env_id}/group/roles' \
  -H 'authorization: Bearer API_SECRET_KEY'
...
```bash
curl 'https://api.permit.io/v2/facts/{project_id}/{env_id}/groups/users/{user_id}' \
    -H 'authorization
