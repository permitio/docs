...
flowchart TB
    subgraph 2
        marketing[marketing<br>resource type] --> social_media[social_media<br>resource instance] --> editor[editor<br>resource role]
    end
    subgraph 1
        permit[company<br>resource type] --> employees[Acme<br>resource instance] --> member[member<br>resource role]
    end

    employees -->|company_group<br>relationship| social_media
    permit -->|comany_group<br>relation| marketing
    editor -->|role derivation| member
```
...
```bash
curl 'https://api.permit.io/v2/facts/{project_id}/{env_id}/groups/{group_resource_type_key}/users/{user_id}' \
    -H 'authorization
