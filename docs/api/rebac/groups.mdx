...
Say we have a Group and inside the group we have Support team and we also have Organization 1. Our goal is to allow all members of the Support ****Team to be members in the Org1 and in all of his related instances (like if we have brand 1 and brand 2 that related to Org 1)
...
As mentioned above, we also have a `Org1` resource instance.
This resource instance supports the role `member`.
...
Now, we want to create the connection between those two group instance.
...
Once we assign the `Org1` group instance  to `Support` group instance,  the following will be automated by the API:
...
Say we have a Group and inside the group, we have a Support team and Organization 1. Our goal is to allow all members of the Support **Team** to be members in Org1 and in all of his related instances (like if we have brand 1 and brand 2 that are related to Org 1)

For this purpose, we will first create a group named Group. The group, group is a resource, while each specific group inside (Such as the support team) is a resource instance.

Users are assigned to the support team via the member role.

As mentioned above, we also have an Org1 resource instance. This resource instance supports the role member.
