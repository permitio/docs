...
:::info How it Works:
Say we create a marketing group and assign a `member` role to it, we then assign users to the Group via a `member` role. The API then [derives](https://docs.permit.io/how-to/build-policies/rebac/overview/#role-derivation) other roles based on this assignment (See example below)
:::
...
As mentioned above, we also have a `social-media` resource, and a `training_video` - which is a resource instance of the `social-media` type. This resource instance supports the role `editor`.
...
Create the `social-media` resource:
...
curl 'https://api.permit.io/v2/facts/{project_id}/{env_id}/group/{group_resource_type_key}/roles' \
 -H 'authorization: Bearer API_SECRET_KEY'
 --data-raw '{"resource": "socail_media", "resource_instance": "training_video", "role": "editor", tenant:"business"}' \
```
