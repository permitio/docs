...
:::info Supported
Google Workspace supports SAML 2.0 and works with Permit.io. These instructions are for Google Workspace admin accounts.
:::

## Prerequisites

- Admin access to **Google Admin Console** (`https://admin.google.com`)
- Your **Permit.io Organization ID** - retrieve it via the [List Organizations API](https://api.permit.io/v2/redoc#tag/Organizations/operation/list_organizations) or from Permit.io dashboard → Settings → Organization
...
3. Click **Add app** → **Add custom SAML app** (not OIDC)
...
**On Windows:**
Open the `.cer` file, go to Details → Copy to File → Base64-encoded X.509
...
- Go to Google Admin → **Apps** → Your app → **User access**
...
**Solution:**
- Go to Google Admin → **Apps** → Your app → Click **SAML attribute mapping**
...
1. In Google Admin, go to your SAML app → **User access**
...
- 📚 Main SAML SSO Guide: [SAML SSO](/authentication/saml-sso)
