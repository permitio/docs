...
## 1. Create the policy the example app expects \{#create-the-policy-the-app-expects}
...
## 2. Run the example app \{#run-the-example-app}
...
4. Read the terminal that runs the app. `pages/api/auth/permit.js` logs one line per check. For a user with the `Stranger` role and the permissions from [1. Create the policy the example app expects](#create-the-policy-the-app-expects), the two checks log:
...
If step 2 shows no user, check that `PERMIT_IO_KEY` belongs to the same environment where you created the `SuperTokens` tenant, and that the PDP container from step 5 of [2. Run the example app](#run-the-example-app) is still running.
