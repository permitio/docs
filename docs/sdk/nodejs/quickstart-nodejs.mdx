...
This example creates a `repository` resource with `clone`, `push`, and `view` actions. The method is `permit.api.resources.create()`:
...
`permit.api.syncUser()` is deprecated. In new code, call `permit.api.users.sync()` with the same user object. For more options, see [Sync users](/how-to/sync-users).

### Assign a role to a user

A role assignment gives a user a role in a tenant. In this example, `user` is the user key and `role` is the role key. `resource_instance` is optional: set `resource_instance` to assign a role on one resource instance, such as `document:1`, and omit `resource_instance` to assign the role in the whole tenant.
