...
The example uses a client named `Permit`, created with `permit.NewPermit()` as shown in [Check permissions with the Go SDK](/sdk/golang/quickstart-golang), and a `ctx` of type `context.Context`. The call fetches the first 10 role assignments of `user-key` in the `default` tenant:
