...
| `Role_assignments` | `ICollection<UserRoleCreate>?` | No | Roles that Permit assigns to the user as it creates the user. Each `UserRoleCreate` has `Role` (a role key or ID), `Tenant` (a tenant key or ID), and `Resource_instance` (a resource instance key in the format `resource_type:instance_key`, or a resource instance ID). |
