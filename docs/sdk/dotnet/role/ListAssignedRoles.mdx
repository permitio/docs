...
The example uses a client named `permitClient`, created with `new Permit(...)` as shown in [Check permissions with the .NET SDK](/sdk/dotnet/quickstart-dotnet). `userId` holds the key of the user. Pass only `userId`. The second positional parameter is `roleId`, so a tenant key passed in that position has no effect:
