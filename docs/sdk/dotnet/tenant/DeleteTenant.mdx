...
The example uses a client named `permitClient`, created with `new Permit(...)` as shown in [Check permissions with the .NET SDK](/sdk/dotnet/quickstart-dotnet). `tenantId` holds the key or ID of the tenant. `DeleteTenant` returns a `Task` with no result:
