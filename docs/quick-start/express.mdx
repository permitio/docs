...
Now to check other way around, create a new user that is not assigned the **Author** role. To create a new user, use permit cli command `permit api sync user`.

The command will prompt you to enter the **User Key**, which can be an email, followed by **Assigned Roles**. A dropdown will appear with all available roles in the project. Select the **Reader** role.

![permit api sync user](/img/quick-start-guide/permit-api-sync-user.png)

Now, `curl` the `/posts` endpoint with the new user.
