...
As shown in the image above, the user is permitted to access the resource. Now to check other way around, `curl` the `/posts` endpoint with a different user that is not assigned the **Author** role.
