...
### Setting up the Express.js with Permit.io

Let's start by setting up the Express.js application. We will create a new file called `server.js` and add the following code to it.
...
We have created an `express` to create the server, `body-parser` to parse the request body, and `Permit` from `permitio` to use the Permit SDK.

Instance of the `Permit` class needs two environment variables `PERMIT_API_KEY` that is the API key we copied from the Permit dashboard, and the `PDP_URL` is the URL of the PDP server. We will set this up later.
...
Now, let's create the endpoints for the Express.js application: 
...
- The first endpoint is `/sync-user`, which will sync the user with Permit.io. 
- The second endpoint is `/create-tenant`, which will create a tenant with Permit.io.
...
To check the access, we will create a new endpoint `/check-permission` in our application. This endpoint will check if the user has access to the resource. Add the following code to the `server.js` file.
