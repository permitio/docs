...
- The `/posts` endpoint checks if a user has access to the **Post** resource.

> **Assigning a Role Programmatically**
>
> After registering the user, you can assign them a role programmatically using the `assignRole` method. In this example, the user is assigned the `Author` role in the `default` tenant. You can change the role or tenant as needed for your application.
