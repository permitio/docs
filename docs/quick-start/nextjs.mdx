---
title: Implementing Fine-grained Authorization in Next.js Application
description: "Add fine-grained authorization to a Next.js application with Permit.io, from policy setup to role assignment and enforcement."
sidebar_position: 4
---
...
Integrate fine-grained authorization into your Next.js application to build secure, scalable web apps where users can access features and content based on their roles and permissions.

This guide shows you how to add fine-grained authorization to a Next.js application using Permit.io. You will set up a policy for creating posts and comments, register users, assign them roles (like Author, Moderator, or Reader), and enforce permissions directly in your app's API routes and middleware.

By the end, you will have a working example where only authorized users can access protected endpoints, and you will understand how to integrate Permit.io into your own Next.js applications.
...
- [Permit.io](https://www.permit.io) account (Follow [this](/quickstart) guide to create an account)

## Policy Configuration

Before integrating Permit.io with your Next.js application, you need to configure a policy. If you already have a policy configured, you can skip this section and proceed directly to the [integration guide](#integration-guide).
...
Now that you have a policy configured, you can integrate Permit.io with your Next.js application. The following steps will guide you through the integration process.
...
To register a user, create an endpoint that registers a user inside the `route.ts` file.
...
To demonstrate the registration process, use the `curl` command to make a request to the `/api/register` endpoint. Register two users **John** and **Emma**. Both of these users will be automatically assigned the **Reader** role.
...
Since the blog application can have various elements like creating a post, commenting, editing, etc., create a new endpoint `/posts` to demonstrate access control. Only users with the **Author** role can create a post. The recommended approach is to use middleware to check if the user has authorization to the resource.
...
To enforce and test the middleware, create a new API route. Create a new file called `route.ts` under `src/app/api/protected/posts.ts` and add the following code:
...
In this guide, you learned how to integrate Permit.io with Next.js. You created a simple Next.js application that syncs users and tenants with Permit.io. You also created endpoints to verify user access to resources. You used the Permit SDK to enforce policies and check authorization. You also set up the PDP server locally using Docker.

## Further Reading

- [Node.js SDK Documentation](/sdk/nodejs/quickstart-nodejs) - Explore advanced features and API methods
- [Embeddable User Management](/embeddable-uis/element/user-management) - Add pre-built UI components for user management
- [Frontend Authorization with CASL](/integrations/feature-flagging/casl) - Implement fine-grained permissions in your React components
