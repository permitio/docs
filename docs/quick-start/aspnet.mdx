...
The Permit client reads the API key from the `PERMIT_API_KEY` environment variable. Set that variable to your environment API key from [2. Get your API key](#2-get-your-api-key). The client connects to the PDP at `http://localhost:7766`, the address from [3. Run the PDP](#setting-up-the-pdp-server).
...
Set the API key environment variable and start the app, replacing `<YOUR_API_KEY>` with your API key:
