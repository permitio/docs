...
:::warning Keep app.run() at the end of app.py
Python runs `app.py` from top to bottom, and `app.run()` blocks until the server stops. If the `if __name__ == "__main__":` block comes before the `/posts` route, Flask starts without that route and `POST /posts` returns `404`. Move the `if __name__ == "__main__":` block to the end of the file.
:::
