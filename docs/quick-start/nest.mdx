...
Once you have a NestJS application set up, we can integrate Permit.io with it. First, we need to install the Permit SDK. To install the SDK, execute the below command in your terminal.
...
Here, we are importing the `Permit` class from the `permitio` package and creating a new instance of it. We are passing the API key and PDP URL to the constructor. The API key is stored in the environment variable `PERMIT_API_KEY` and the PDP URL is stored in the environment variable `PDP_URL`.
