...
# MCPermit - MCP Permissions Architecture Review Guide
## Executive Summary

MCPermit delivers **permissions-as-a-service** for any Model Context Protocol (MCP) server—hosted or self-managed—by inserting a simple proxy/gateway (or middleware) between AI agents and the resources they invoke. It supplies:

* **Fine-grained ReBAC** (relationship-based access control) powered by Permit.io, OPA, and OPAL.
* **Five-stage authentication & authorization** that binds the user, agent, MCP server, and downstream service identities into a provable chain of trust.
* **Comprehensive auditing and anomaly detection** covering every call, every scope, every actor.
* **Flexible delivery models** (hosted gateway, side-car middleware, or MCP-hosting platform) that share identical policy semantics.
...
* **Permit.io PDP** runs embedded (WASM) for µ-latency or remotely via OPAL for shared caching.
...
### 4.1 Levels of Policy

1. **Organization Policy** – controls *agent-type ↔ server-type* pairings (e.g. `chat → personal`).
...
MCPermit provides policies to control access to MCP servers using Permit.io (Which in turn is built on top of OPA / OPAL and a permissions graph inspired by Google Zanzibar).
MCPermit automatically generates ReBAC (Relationship based Access Control)  policies based on:
...
```
Note:
- OBO (On Behalf Of ) flow - Agent deriving access from user profile
- RI: Resource Instance

---

## 5 · Authentication & Token Taxonomy
...
| **5** | Agent OBO User → Service | RFC 8693 token exchange          | User context downstream |

Gateway issues composite JWTs, signing only the minimal context each hop needs.

---

## 6 · Sequence Flows

### 6.1 First-Run Delegation
...
| **OPA Provenance**     | Sigstore signatures on every bundle                |
