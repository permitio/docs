...
Note:
Clicking **Authorize Agent** will authorize the agent to access the MCP server, and will redirect to the external tools' OAuth flow, and then back to the agent's configured callback URL.
