...
The Food Delivery System is a Nuxt.js application that demonstrates how to implement different types of authorization in a real-world scenario. The system includes:
...
In this guide, you'll build a food delivery system with:
...
The full code is available at: https://github.com/permitio/permit-nuxt-example
...
- For free delivery orders, rider must have 500+ rides (`ABAC`)
...
Let's get your development environment set up. We'll guide you through installing the necessary tools and configuring your project step by step.
...
[Role-Based Access Control (RBAC)](/how-to/build-policies/rbac/overview/) is usually the first line of authorization strategy to implement. The Food Delivery System uses `RBAC` first for all actions. In the context of resources, we enforce the following policies.
...
![Food Delivery System Roles](/img/modeling/food-delivery-system-example-using-nuxt/rbac-roles.png)
...
The Food Delivery System app uses promotional offers to show how to use `ABAC`. Orders with at least 500 costs are eligible for free delivery. Also, only riders with at least 500 number of rides can deliver these promotional orders. We can rephrase these `ABAC` policies as follows:
...
[Relationship-Based Access Control (ReBAC)](/how-to/build-policies/rebac/overview/) uses associations or links between users and resources to grant or deny permissions. `ReBAC` leverages relationships in the overall system for access control. The Food Delivery System application uses `ReBAC` to enforce the following policies:
...
The Food Delivery System syncs users with the Permit project in the handlers of the `/users` endpoints as follows:
...
In the Food Delivery System code, we sync orders with the Permit project. We also assign the related instance role on the "Order" resource where appropriate.
