...
You need a valid Docker installation to test “allow”s and “deny”s. We use [Docker](https://www.docker.com/) to run the [Policy Decision Point (PDP)](/concepts/pdp/overview) for Nuxt / Vue Permissions.
...
When using the check function, you can also supply more arguments like user or resource attributes. Ordinarily, you won't need to do this as you were already syncing that data with Permit. But if you need a one-time override, you can do so with the check function. 

Additionally, you should use the result of the check function to either allow or deny access for the target action. In the example above, we simply reject the HTTP call we an appropriate message.
