...
This guide uses a food ordering system to demonstrate how to combine RBAC, ABAC, and ReBAC to build robust and granular-level authorization policies. With minimal code, we'll leverage Permit’s policy editor, SDKs, and APIs to implement flexible and scalable access control.
...
a. The user needs to have the `rider` role to make a delivery (RBAC).  
b. We only allows riders to deliver orders that were assigned to them (ReBAC).  
c. If the involved order has a free delivery, we also check the rider’s number of rides (ABAC).
...
Open http://localhost:3000 in your browser. Every action you take will not be authorized. You have to configure the necessary policies in the Permit UI. Remember to create, delete, assign, or unassign roles to test user IDs from the app.
