...
description: "Cloud vs Sidecar PDP"
---

If you've reached this point, it's likely that you have successfully [completed the onboarding guide](/getting-started/create-your-account/),
connected your application with Permit, and checked for permissions. Now, let's dive into the different deployment
options available for incorporating Permit into your app.
...
| [Policy Enforcement Point(PEP)](/overview/permit-basics#pep) | MUST     | Otherwise what's the point? 😜😅 These can be [Frontend code, Backend code](/getting-started/quickstart#check-for-permissions-using-the-sdk), [reverse proxy, or API Gateway](/integrations/gateways/overview) |
| [Policy Decision Point (PDP)](/overview/permit-basics#pdp)   | OPTIONAL | Permit can host it for you. But it's recommended to use locally in production.                                                                                                                                 |
...
In this slightly more advanced deployment, you run multiple instances of the same PDP (with the same [environnement key](https://docs.permit.io/features/projects-and-env#environments)) behind a load-balancer and direct your services to it.
