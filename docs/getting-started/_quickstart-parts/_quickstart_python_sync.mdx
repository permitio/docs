...
:::warning The example targets Python SDK version 1.0.0rc1
The example installs `permit==1.0.0rc1` and calls `permit.write()`. The `Permit` client in current versions of the `permit` package has no `write()` method, so the example fails with a current SDK. With a current SDK, sync users with `permit.api.users.sync()`. See [Sync users](/how-to/sync-users).
:::
