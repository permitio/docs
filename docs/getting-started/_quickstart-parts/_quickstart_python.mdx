...
The app checks whether `john@smith.com` can `read` a `document`. If the policy allows the action, the app returns HTTP `200` with a JSON `result` message that contains `is PERMITTED`. Otherwise, the app returns HTTP `403` with a message that contains `is NOT PERMITTED`.
