...
1. Create a directory with an empty .NET console project. Run this shell command in your terminal:
...
Call `await permit.Check()` with three arguments: the user key, the action key, and the resource key. `permit.Check()` returns `true` when the policy allows the action, and `false` otherwise. In this example, `user` is a `UserKey` object, as in the full example app below:
...
To check an ABAC policy, pass a `UserKey` and a `ResourceInput` with attributes. The `attributes` parameter of `ResourceInput` has the type `Dictionary<string, dynamic>`. In this example, replace `userId`, `resource`, `tenant`, and `action` with a user key, resource key, tenant key, and action key from your environment:
