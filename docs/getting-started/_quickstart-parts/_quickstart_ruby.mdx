...
3. Create a `Permit` client with `Permit.new(token, pdp_url)`. Replace `TOKEN` with your environment API key. The second argument is the PDP URL, including the scheme, for example `http://localhost:7766`. The PDP URL is optional and defaults to `http://localhost:7766`.
