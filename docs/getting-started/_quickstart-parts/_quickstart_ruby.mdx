...
:::warning A PDP error raises an exception instead of returning false
...
In a multi-tenant application, pass the tenant key in the `tenant` field of the resource hash, as `resource_hash` does in the hash example. To look up the keys of your tenants, call the [list tenants API](https://api.permit.io/v2/redoc#tag/Tenants/operation/list_tenants). In this example, replace `user`, `action`, `resource`, and `tenant` with a user key, action key, resource key, and tenant key from your environment:
