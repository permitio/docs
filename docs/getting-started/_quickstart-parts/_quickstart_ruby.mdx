...
3. Create a `Permit` client with `Permit.new(token, pdp_url)`. Replace `<YOUR_API_KEY>` with your environment API key. The second argument is the PDP URL, for example `http://localhost:7766`. The PDP URL is optional and defaults to `http://localhost:7766`.
...
If the user's role grants `read` on `document`, the page returns HTTP `200` with a JSON `result` of `john@permit.io is PERMITTED to read document!`. Otherwise, the page returns HTTP `403` with `john@permit.io is NOT PERMITTED to read document`.
